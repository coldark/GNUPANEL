...
if($estado==PGRES_CONNECTION_OK)
	...
	my $comandar = "/usr/lib/postgresql/9.1/bin/pg_dump -Fc -O -f $fila $base_postgres ";
	$control = $control + system($comandar);
	}
    }

#$comandar = "/bin/tar --remove-files -c -O -z -C $directorio_backup $dominio";
$comandar = "/bin/tar -c -O -z -C $directorio_backup $dominio";
$comandar_rm = "/bin/rm -f -R ".pone_barra($directorio_backup).$dominio;
