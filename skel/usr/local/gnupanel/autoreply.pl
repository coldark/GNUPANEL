...
sub envia_correo
{
    ...
    my $comando = "|/usr/bin/maildrop -d ".$direccion;

    open(RECIPIENTE,$comando);
    print RECIPIENTE $original;
    close RECIPIENTE;
...
while($renglon = <ENTRADA>)
    {
    $entrada = $entrada.$renglon;
    }

envia_correo($recipient,$sender,$entrada);
...
if($estado == PGRES_CONNECTION_OK)
	...
	{
	encola_autorespuesta($conexion,$recipient,$sender);
	}
    }
else
    {
