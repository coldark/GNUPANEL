...
$SIG{'KILL'} = \&senal_de_kill;
    ...
    if ( $child_pid != 0)
	{
	    print "Inicializando el medidor de trafico de HTTPS \n";
	    exit(0);
	}
    else
	{
...
#	    undef($gid);
	    ...
	    $file = File::Tail->new(name=>$archivo_log_https,interval=>1,maxinterval=>1,adjustafter=>1);
	    $conexion = NULL;
		...
		if($estado==PGRES_CONNECTION_OK)
			...
			{
			    chop($linea);
	    		    analiza_https($linea,$conexion,$logueo);
			}
		    }
		else
		    {
