...
function lista_usuarios_ftp($comienzo)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id,userid FROM gnupanel_proftpd_ftpuser WHERE id_dominio = $id_usuario ORDER BY userid LIMIT $cant_max_result OFFSET $comienzo";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function cambia_usuario_ftp($id,$pasaporte)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db ";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "UPDATE gnupanel_proftpd_ftpuser SET passwd = '$pasaporte_crypt' WHERE id = $id AND id_dominio = $id_usuario ";
    $res_consulta = pg_query($conexion,$consulta);
...
function verifica_cambia_usuario_ftp($pasaporte_0,$pasaporte_1)
{
	global $escribir;
	$retorno = NULL;
...
function cambia_usuario_ftp_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function cambia_usuario_ftp_1($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	print "<table width=\"60%\" > \n";
		...
		genera_fila_formulario("id_ftp",$id_ftp,'hidden',NULL,true);
...
function cambia_usuario_ftp_2($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$comienzo = $_POST['comienzo'];
	$usuario_data = dame_usuario_ftp($id_ftp);
	$checkeo = verifica_cambia_usuario_ftp($pasaporte_0,$pasaporte_1);

	if($checkeo)
	{
	cambia_usuario_ftp_1($procesador,$checkeo);
	}
	else
	{
	$escriba = "";
	if(cambia_usuario_ftp($id_ftp,$pasaporte_0))
		{
...
function cambia_usuario_ftp_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		cambia_usuario_ftp_1($nombre_script,NULL);
		break;

		case "2":
		cambia_usuario_ftp_2($nombre_script,NULL);
		break;

		default:
		cambia_usuario_ftp_0($nombre_script,NULL);
	}
}
