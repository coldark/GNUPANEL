...
function dame_directorios($id_apache)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT documentroot from gnupanel_apacheconf WHERE id_dominio = $id_usuario AND id_apache = $id_apache ";
    $res_consulta = pg_query($conexion,$consulta);
    $documentroot = pg_fetch_result($res_consulta,0,0);
    $largo = strlen($documentroot);
    $comando = "/usr/local/gnupanel/bin/listado.gnupanel ".$documentroot;
...
function lista_subdominios($comienzo)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id_apache,id_subdominio,subdominio,es_ssl,documentroot FROM gnupanel_apacheconf WHERE id_dominio = $id_usuario ORDER BY id_subdominio ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function verifica_agregar_dir_prot($id_apache,$directorio,$descripcion)
{
	global $escribir;
	$retorno = NULL;
	if(!verifica_dato($descripcion,NULL,true,true)) $retorno = $escribir['carac_inv']." ";
	if(existe_dir_prot($id_apache,$directorio)) $retorno = $escribir['existe']." ";
	if(strlen($descripcion)==0) $retorno = $escribir['sin_carac']." ";
	return $retorno;
}
...
function agregar_dir_prot($id_apache,$directorio,$descripcion)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
...
function agregar_dir_prot_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	print "<td width=\"30%\" > \n";
	$escriba = $escribir['proteger_directorio'];
...
function agregar_dir_prot_1($procesador,$mensaje)
{
    global $idioma;
    global $escribir;
    global $plugin;
...
function agregar_dir_prot_2($procesador,$mensaje)
{
    global $idioma;
    global $escribir;
    global $plugin;
	...
	$checkea = verifica_agregar_dir_prot($id_apache,$directorio,$descripcion);

	if($checkea)
	{
	agregar_dir_prot_1($procesador,$checkea);
	}
	else
	{
	$chequeo = agregar_dir_prot($id_apache,$directorio,$descripcion,$directorio);
	print "<div id=\"formulario\" > \n";
...
function agregar_dir_prot_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		agregar_dir_prot_1($nombre_script,NULL);
		break;
		case "2":
		agregar_dir_prot_2($nombre_script,NULL);
		break;
		default:
		agregar_dir_prot_0($nombre_script,NULL);
	}
}
