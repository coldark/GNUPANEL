...
if(strlen($subdominio)==0) $retorno = true;
return $retorno;
}
...
function borrar_subdominio($id_subdominio,$es_ssl)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $consulta = "DELETE FROM gnupanel_apacheconf WHERE id_dominio = $id_usuario AND id_subdominio = $id_subdominio AND es_ssl = $es_ssl ";
    $res_consulta = pg_query($conexion,$consulta);
    $checkeo = $checkeo && $res_consulta;

    $consulta = "SELECT * from gnupanel_apacheconf WHERE id_dominio = $id_usuario AND id_subdominio = $id_subdominio ";
    $res_consulta = pg_query($conexion,$consulta);
    $checkeo = $checkeo && $res_consulta;
    ...
    if($cantidad==0)
	{
	$consulta = "DELETE FROM gnupanel_pdns_records WHERE domain_id = $id_usuario AND id = $id_subdominio ";
	$res_consulta = pg_query($conexion,$consulta);
	$checkeo = $checkeo && $res_consulta;
	}
...
function borrar_subdominio_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function borrar_subdominio_1($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$procesador_inc = $procesador."&#063;seccion&#061;$seccion&#038;plugin&#061;$plugin";
	$variables = array();
	$variables['id_subdominio'] = $id_subdominio;
	$variables['comienzo'] = $comienzo;
	$variables['es_ssl'] = $subdominio_data['es_ssl'];
...
function borrar_subdominio_2($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	if(borrar_subdominio($id_subdominio,$es_ssl))
	{
...
function borrar_subdominio_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		borrar_subdominio_1($nombre_script,NULL);
		break;

		case "2":
		borrar_subdominio_2($nombre_script,NULL);
		break;

		default:
		borrar_subdominio_0($nombre_script,NULL);
	}
}
