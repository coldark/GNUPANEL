...
function cantidad_subdominios()
{
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT subdominio from gnupanel_apacheconf WHERE id_dominio = $id_usuario AND redirigir = 1 ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function lista_subdominios($comienzo)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id_apache,id_subdominio,subdominio,es_ssl FROM gnupanel_apacheconf WHERE id_dominio = $id_usuario AND redirigir = 1 ORDER BY id_subdominio ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function quitar_redireccion($id_apache)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
	...
	$consulta = "UPDATE gnupanel_apacheconf SET redirigir = 0, dominio_destino = NULL, estado = 1 WHERE id_apache = $id_apache AND id_dominio = $id_usuario ";
	$res_consulta = pg_query($conexion,$consulta);
...
function quitar_redireccion_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function quitar_redireccion_1($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	print "<td width=\"40%\" > \n";
	$escriba = $escribir['redireccion'];
	print "$escriba \n";
	print "</td> \n";
	print "<td width=\"60%\" > \n";
	$escriba = $dominio_destino;
	print "$escriba \n";
	print "</td> \n";
	print "</tr> \n";
...
function quitar_redireccion_2($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$id_apache = $_POST['id_apache'];
	$escriba = NULL;
	if(quitar_redireccion($id_apache))
	{
...
function quitar_redireccion_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		quitar_redireccion_1($nombre_script,NULL);
		break;

		case "2":
		quitar_redireccion_2($nombre_script,NULL);
		break;

		default:
		quitar_redireccion_0($nombre_script,NULL);
	}
}
