...
function dame_descripcion($id_apache,$directorio)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $consulta = "SELECT descripcion from gnupanel_apache_dir_prot WHERE id_subdominio = $id_apache AND directorio = '$directorio' ";
...
function dame_directorios($id_apache)
{
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT directorio from gnupanel_apache_dir_prot WHERE id_subdominio = $id_apache ";
    $res_consulta = pg_query($conexion,$consulta);

    while($retorno = pg_fetch_assoc($res_consulta))
	{
	$result[] = $retorno['directorio'];
	}
...
function desproteger_directorio_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	print "<td width=\"30%\" > \n";
	$escriba = $escribir['desproteger_directorio'];
...
function desproteger_directorio_1($procesador,$mensaje)
{
    global $idioma;
    global $escribir;
    global $plugin;
...
function desproteger_directorio_2($procesador,$mensaje)
{
    global $idioma;
    global $escribir;
    global $plugin;
	...
	$descripcion = dame_descripcion($id_apache,$directorio);
	$chequeo = desproteger_directorio($id_apache,$directorio);
	print "<div id=\"formulario\" > \n";
...
function desproteger_directorio_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		desproteger_directorio_1($nombre_script,NULL);
		break;
		case "2":
		desproteger_directorio_2($nombre_script,NULL);
		break;
		default:
		desproteger_directorio_0($nombre_script,NULL);
	}
}
