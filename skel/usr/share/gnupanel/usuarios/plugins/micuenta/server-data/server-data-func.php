...
function dame_servidor_usuario($id_usuario)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $subconsulta_0 = "(SELECT id_servidor from gnupanel_ips_servidor WHERE id_ip=(SELECT id_ip from gnupanel_reseller WHERE id_reseller=(SELECT cliente_de FROM gnupanel_usuario WHERE id_usuario=$id_usuario)))";
...
function server_data_0($nombre_script,$mensaje)
{
	...
	$id_usuario = $_SESSION['id_usuario'];
	$servidorete = dame_servidor_usuario($id_usuario);
