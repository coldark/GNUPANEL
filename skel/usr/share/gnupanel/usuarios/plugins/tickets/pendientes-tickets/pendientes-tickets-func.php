...
function cantidad_tickets_pendientes()
    ...
    $consulta = "SELECT * from gnupanel_tickets_usuarios WHERE id_usuario = $id_usuario AND atendido = 0 AND conforme = 0 ";
...
function lista_tickets_pendientes($comienzo)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id,id_ticket,asunto FROM gnupanel_tickets_usuarios WHERE id_usuario = $id_usuario AND atendido = 0 AND conforme = 0 ORDER BY id LIMIT $cant_max_result OFFSET $comienzo";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
