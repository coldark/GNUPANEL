...
function verifica_wordpress($directorio,$base_mysql,$usuario_mysql,$password_mysql,$sitio_wordpress,$correo_wordpress,$admin_wordpress,$password_wordpress,$password_wordpress_1,$dominio,$subdominio_data)
{
	global $escribir;
	$retorno = NULL;
	$tipo = 1;
	...
	if(!verifica_correo($correo_wordpress)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($admin_wordpress,NULL)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($sitio_wordpress,NULL,true,true)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($password_wordpress,NULL)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($password_wordpress_1,NULL)) $retorno = $escribir['carac_inv']." ";
	...
	if($password_wordpress != $password_wordpress_1) $retorno = $escribir['distintos']." ";
	if(strlen($password_wordpress) < 8) $retorno = $escribir['pocos_carac']." ";
	$directorio_wordpress = pone_barra(pone_barra($subdominio_data['documentroot']).$directorio);
	$archivo_existe_wordpress = $directorio_wordpress."gnupanel-wordpress.php";
	if(is_file($archivo_existe_wordpress)) $retorno = $escribir['ya_hay_wordpress']." ";
	if(trim($directorio," /")=="gnupanel") $retorno = $escribir['dir_no_perm']." ";
	if(trim($directorio," /")=="webmail") $retorno = $escribir['dir_no_perm']." ";
	return $retorno;
}

function directorio_vacio($directorio_wordpress,$directorio_raiz)
{
global $escribir;
$retorno = NULL;
$archivos = NULL;

if(is_file(rtrim($directorio_wordpress,"/ "))) $retorno = $escribir['direc_es_archivo'];

if($directorio_wordpress==$directorio_raiz)
	{
	$archivos = scandir($directorio_wordpress);
	...
	{
	if(is_dir($directorio_wordpress))
		{
		$archivos = scandir($directorio_wordpress);
		if(is_array($archivos))
			{
...
function agregar_sql_wordpress($base_mysql,$usuario_mysql,$password_mysql,$password_wordpress,$correo_wordpress,$es_ssl,$dominio_wordpress,$admin_wordpress,$sitio_wordpress)
{
	global $nombre_servidor;
	global $ip_db_servidor;
	global $idioma;
	...
	$fila_sql = "/usr/share/gnupanel/usuarios/plugins/herramientas/wordpress/gnupanel-wordpress-en.sql" ;
	if($idioma=="es") $fila_sql = "/usr/share/gnupanel/usuarios/plugins/herramientas/wordpress/gnupanel-wordpress-es.sql" ;
	$archivo_sql = file_get_contents($fila_sql);
	...
	if(is_array($sqls))
	{
		...
		$consulta = trim(str_replace("URL_SITIO_CON_DIR","$dominio_wordpress",$consulta));
		$consulta = trim(str_replace("CORREO_WEBLOG","$correo_wordpress",$consulta));
		$consulta = trim(str_replace("TITULO_WEBLOG","$sitio_wordpress",$consulta));
		$consulta = trim(str_replace("UNIQUID_WORDPRESS_MD5","$unico_id",$consulta));
		$consulta = trim(str_replace("PASSWORD_MD5","$password_wordpress",$consulta));
...
function wordpress($id_apache,$directorio,$base_mysql,$usuario_mysql,$password_mysql,$sitio_wordpress,$correo_wordpress,$admin_wordpress,$password_wordpress)
{
	global $_SESSION;
	global $nombre_servidor;
	global $ip_db_servidor;
	...
	$id_usuario = $_SESSION['id_usuario'];
	$tipo = 1;
	$password_wordpress_crypt = md5($password_wordpress);
	...
	{
		$dominio_wordpress = $prefijo.$dominio."/".$directorio;
	}
	else
	{
		$dominio_wordpress = $prefijo.$subdominio_data['subdominio'].".".$dominio."/".$directorio;
	}

	$checkeo = agregar_base_datos_my($id_usuario,$base_mysql,$usuario_mysql,$password_mysql,$tipo);
	$checkeo = $checkeo && agregar_sql_wordpress($base_mysql,$usuario_mysql,$password_mysql,$password_wordpress_crypt,$correo_wordpress,$subdominio_data['es_ssl'],$dominio_wordpress,$admin_wordpress,$sitio_wordpress);

	$directorio_raiz = pone_barra($subdominio_data['documentroot']);
	$directorio_wordpress = pone_barra(pone_barra($subdominio_data['documentroot']).$directorio);
	$lenguaje = "";
	if($idioma=="es") $lenguaje = "es_ES";

	$comando = "/usr/local/gnupanel/bin/wordpress-inst.sh $directorio_wordpress $directorio_raiz $host_mysql $base_mysql $usuario_mysql $password_mysql $lenguaje ";
	
	$sistema = system($comando);
	if(!$checkeo) borrar_base_datos_my($base_mysql,$usuario_mysql);
	if($checkeo) $checkeo = $dominio_wordpress;
...
function wordpress_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	print "<td width=\"30%\" > \n";
	$escriba = $escribir['inst_wordpress'];
...
function wordpress_1($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$sitio_wordpress = $_POST['sitio_wordpress'];
	$correo_wordpress = $_POST['correo_wordpress'];
	$admin_wordpress = "admin";
	$password_wordpress = $_POST['password_wordpress'];
	$password_wordpress_1 = $_POST['password_wordpress_1'];
	...
	genera_fila_formulario("sitio_wordpress",$sitio_wordpress,'text',NULL,!$mensaje,true,true);
	genera_fila_formulario("correo_wordpress",$correo_wordpress,'text_correo',NULL,!$mensaje);
	genera_fila_formulario("password_wordpress",$password_wordpress,'password',NULL,!$mensaje);
	genera_fila_formulario("password_wordpress_1",$password_wordpress_1,'password',NULL,!$mensaje);
	genera_fila_formulario("admin_wordpress",$admin_wordpress,'hidden',NULL,true);
	...
	genera_fila_formulario("inst_wordpress",NULL,'submit',NULL,true);
...
function wordpress_2($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
;
	...
	$sitio_wordpress = $_POST['sitio_wordpress'];
	$correo_wordpress = $_POST['correo_wordpress'];
	$admin_wordpress = $_POST['admin_wordpress'];

	$password_wordpress = $_POST['password_wordpress'];
	$password_wordpress_1 = $_POST['password_wordpress_1'];

	
	$checkeo = verifica_wordpress($directorio,$base_mysql,$usuario_mysql,$password_mysql,$sitio_wordpress,$correo_wordpress,$admin_wordpress,$password_wordpress,$password_wordpress_1,$dominio,$subdominio_data);
	
	if($checkeo)
	{
	wordpress_1($procesador,$checkeo);
	}
	else
	{
	$directorio_raiz = pone_barra($subdominio_data['documentroot']);
	...
	$cuidado = directorio_vacio($directorio_wordpress,$directorio_raiz);
	print "<div id=\"formulario\" > \n";
	print "<ins> \n";
	print "<form method=\"post\" action=\"$procesador&#063;seccion&#061;$seccion&#038;plugin&#061;$plugin\" > \n";
	print "<table width=\"80%\" > \n";
	...
	genera_fila_formulario("sitio_wordpress",$sitio_wordpress,'hidden',NULL,true);
	genera_fila_formulario("correo_wordpress",$correo_wordpress,'hidden',NULL,true);
	genera_fila_formulario("admin_wordpress",$admin_wordpress,'hidden',NULL,true);
	genera_fila_formulario("password_wordpress",$password_wordpress,'hidden',NULL,true);
	genera_fila_formulario("password_wordpress_1",$password_wordpress_1,'hidden',NULL,true);
...
function wordpress_3($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$checkeo = wordpress($id_apache,$directorio,$base_mysql,$usuario_mysql,$password_mysql,$sitio_wordpress,$correo_wordpress,$admin_wordpress,$password_wordpress);

	$escriba = NULL;
	if($checkeo)
	{
		$acceder = $checkeo;
		$acceder_admin = pone_barra($checkeo)."wp-admin";
...
function wordpress_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		wordpress_1($nombre_script,NULL);
		break;

		case "2":
		wordpress_2($nombre_script,NULL);
		break;

		case "3":
		wordpress_3($nombre_script,NULL);
		break;

		default:
		wordpress_0($nombre_script,NULL);
	}
}
