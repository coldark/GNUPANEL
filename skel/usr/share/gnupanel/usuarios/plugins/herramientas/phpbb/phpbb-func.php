...
function verifica_phpbb($directorio,$base_mysql,$usuario_mysql,$password_mysql,$sitio_phpbb,$correo_phpbb,$admin_phpbb,$password_phpbb,$password_phpbb_1,$dominio,$subdominio_data)
{
	global $escribir;
	$retorno = NULL;
	$tipo = 1;
	...
	if(!verifica_correo($correo_phpbb)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($admin_phpbb,NULL)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($sitio_phpbb,NULL,true,true)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($password_phpbb,NULL)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($password_phpbb_1,NULL)) $retorno = $escribir['carac_inv']." ";
	...
	if($password_phpbb != $password_phpbb_1) $retorno = $escribir['distintos']." ";
	if(strlen($password_phpbb) < 8) $retorno = $escribir['pocos_carac']." ";
	$directorio_phpbb = pone_barra(pone_barra($subdominio_data['documentroot']).$directorio);
	$archivo_existe_phpbb = $directorio_phpbb."gnupanel-phpbb.php";
	if(is_file($archivo_existe_phpbb)) $retorno = $escribir['ya_hay_phpbb']." ";
	if(trim($directorio," /")=="gnupanel") $retorno = $escribir['dir_no_perm']." ";
	if(trim($directorio," /")=="webmail") $retorno = $escribir['dir_no_perm']." ";
	return $retorno;
}

function directorio_vacio($directorio_phpbb,$directorio_raiz)
{
global $escribir;
$retorno = NULL;
$archivos = NULL;

if(is_file(rtrim($directorio_phpbb,"/ "))) $retorno = $escribir['direc_es_archivo'];

if($directorio_phpbb==$directorio_raiz)
	{
	$archivos = scandir($directorio_phpbb);
...
function agregar_sql_phpbb($base_mysql,$usuario_mysql,$password_mysql,$password_phpbb,$correo_phpbb,$directorio,$es_ssl,$dominio_phpbb,$admin_phpbb)
{
	global $nombre_servidor;
	global $ip_db_servidor;
	global $idioma;
	...
	$checkeo = true;
	$idioma_phpbb = "english";
	if($idioma=="es") $idioma_phpbb = "spanish";
	
	$servidor_db_host = $ip_db_servidor[$nombre_servidor];
	$puerto = '80';
	if($es_ssl==1) $puerto = '443';
	$archivo_sql = file_get_contents("/usr/share/gnupanel/usuarios/plugins/herramientas/phpbb/gnupanel-phpbb.sql");
	...
	if(is_array($sqls))
	{
		...
		$consulta = "INSERT INTO phpbb_smilies (smilies_id, code, smile_url, emoticon) VALUES ( 33, ';)', 'icon_wink.gif', 'Wink')";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$consulta = "INSERT INTO phpbb_smilies (smilies_id, code, smile_url, emoticon) VALUES ( 34, ';-)', 'icon_wink.gif', 'Wink')";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);
		
		$tiempo = time();

		$consulta = "INSERT INTO phpbb_config (config_name, config_value) VALUES ('board_startdate', '$tiempo')";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$consulta = "INSERT INTO phpbb_config (config_name, config_value) VALUES ('default_lang', '$idioma_phpbb')";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$consulta = "UPDATE phpbb_config SET config_value = '$correo_phpbb' WHERE config_name = 'board_email' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);
		
		$directorio = pone_barra_principio(pone_barra($directorio));
		$consulta = "UPDATE phpbb_config SET config_value = '$directorio' WHERE config_name = 'script_path' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$consulta = "UPDATE phpbb_config SET config_value = '$puerto' WHERE config_name = 'server_port' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$consulta = "UPDATE phpbb_config SET config_value = '$dominio_phpbb' WHERE config_name = 'server_name' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$consulta = "UPDATE phpbb_users SET username = '$admin_phpbb', user_password = '$password_phpbb', user_lang = '$idioma_phpbb', user_email = '$correo_phpbb'  WHERE username = 'Admin' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$tiempo = time();
		$consulta = "UPDATE phpbb_users SET user_regdate = $tiempo WHERE username = 'Admin' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);
...
function phpbb($id_apache,$directorio,$base_mysql,$usuario_mysql,$password_mysql,$sitio_phpbb,$correo_phpbb,$admin_phpbb,$password_phpbb)
{
	global $_SESSION;
	global $nombre_servidor;
	global $ip_db_servidor;
	...
	$id_usuario = $_SESSION['id_usuario'];
	$tipo = 1;
	$password_phpbb_crypt = md5($password_phpbb);
	...
	{
		$dominio_phpbb = $dominio_phpbb.$dominio;
	}
	else
	{
		$dominio_phpbb = $dominio_phpbb.$subdominio_data['subdominio'].".".$dominio;
	}

	$checkeo = agregar_base_datos_my($id_usuario,$base_mysql,$usuario_mysql,$password_mysql,$tipo);
	$checkeo = $checkeo && agregar_sql_phpbb($base_mysql,$usuario_mysql,$password_mysql,$password_phpbb_crypt,$correo_phpbb,pone_barra($directorio),$subdominio_data['es_ssl'],$dominio_phpbb,$admin_phpbb);

	$directorio_raiz = pone_barra($subdominio_data['documentroot']);
	$directorio_phpbb = pone_barra(pone_barra($subdominio_data['documentroot']).$directorio);
	$comando = "/usr/local/gnupanel/bin/phpbb-inst.sh $directorio_phpbb $directorio_raiz $host_mysql $base_mysql $usuario_mysql $password_mysql ";
	...
	$checkeo = $prefijo.$dominio_phpbb.pone_barra_principio($directorio);
	}
...
function phpbb_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function phpbb_1($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$sitio_phpbb = $_POST['sitio_phpbb'];
	$correo_phpbb = $_POST['correo_phpbb'];
	$admin_phpbb = $_POST['admin_phpbb'];
	$password_phpbb = $_POST['password_phpbb'];
	$password_phpbb_1 = $_POST['password_phpbb_1'];
	...
	genera_fila_formulario("sitio_phpbb",$sitio_phpbb,'text',NULL,!$mensaje,true,true);
	genera_fila_formulario("correo_phpbb",$correo_phpbb,'text_correo',NULL,!$mensaje);
	genera_fila_formulario("admin_phpbb",$admin_phpbb,'text',NULL,!$mensaje,NULL);
	genera_fila_formulario("password_phpbb",$password_phpbb,'password',NULL,!$mensaje);
	genera_fila_formulario("password_phpbb_1",$password_phpbb_1,'password',NULL,!$mensaje);
	...
	genera_fila_formulario("inst_phpbb",NULL,'submit',NULL,true);
...
function phpbb_2($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
;
	...
	$checkeo = verifica_phpbb($directorio,$base_mysql,$usuario_mysql,$password_mysql,$sitio_phpbb,$correo_phpbb,$admin_phpbb,$password_phpbb,$password_phpbb_1,$dominio,$subdominio_data);
	
	if($checkeo)
	{
	phpbb_1($procesador,$checkeo);
	}
	else
	{
	$directorio_raiz = pone_barra($subdominio_data['documentroot']);
	...
	$cuidado = directorio_vacio($directorio_phpbb,$directorio_raiz);
	print "<div id=\"formulario\" > \n";
	print "<ins> \n";
	print "<form method=\"post\" action=\"$procesador&#063;seccion&#061;$seccion&#038;plugin&#061;$plugin\" > \n";
	print "<table width=\"80%\" > \n";
	...
	genera_fila_formulario("sitio_phpbb",$sitio_phpbb,'hidden',NULL,true);
	genera_fila_formulario("correo_phpbb",$correo_phpbb,'hidden',NULL,true);
	genera_fila_formulario("admin_phpbb",$admin_phpbb,'hidden',NULL,true);
	genera_fila_formulario("password_phpbb",$password_phpbb,'hidden',NULL,true);
	genera_fila_formulario("password_phpbb_1",$password_phpbb_1,'hidden',NULL,true);
...
function phpbb_3($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$checkeo = phpbb($id_apache,$directorio,$base_mysql,$usuario_mysql,$password_mysql,$sitio_phpbb,$correo_phpbb,$admin_phpbb,$password_phpbb);

	$escriba = NULL;
	if($checkeo)
	{
		$acceder = $checkeo;
		$acceder_admin = pone_barra($checkeo)."admin";
...
function phpbb_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		phpbb_1($nombre_script,NULL);
		break;

		case "2":
		phpbb_2($nombre_script,NULL);
		break;

		case "3":
		phpbb_3($nombre_script,NULL);
		break;

		default:
		phpbb_0($nombre_script,NULL);
	}
}
