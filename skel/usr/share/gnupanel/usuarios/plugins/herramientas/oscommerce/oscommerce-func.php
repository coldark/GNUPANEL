...
function verifica_oscommerce($directorio,$base_mysql,$usuario_mysql,$password_mysql,$admin_oscommerce,$password_oscommerce,$password_oscommerce_1,$dominio,$subdominio_data)
{
	global $escribir;
	global $_SESSION;
	$retorno = NULL;
	...
	if(!verifica_dato($admin_oscommerce,NULL)) $retorno = $escribir['carac_inv']." ";
	if(existe_usuario_apache($admin_oscommerce."@".$dominio)) $retorno = $escribir['existe_usuario']." ";
	if(!verifica_dato($password_oscommerce,NULL)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($password_oscommerce_1,NULL)) $retorno = $escribir['carac_inv']." ";
	...
	if($password_oscommerce != $password_oscommerce_1) $retorno = $escribir['distintos']." ";
	if(strlen($password_oscommerce) < 8) $retorno = $escribir['pocos_carac']." ";
	$directorio_oscommerce = pone_barra(pone_barra($subdominio_data['documentroot']).$directorio);
	$archivo_existe_oscommerce = $directorio_oscommerce."gnupanel-oscommerce.php";
	if(is_file($archivo_existe_oscommerce)) $retorno = $escribir['ya_hay_oscommerce']." ";
	if(trim($directorio," /")=="gnupanel") $retorno = $escribir['dir_no_perm']." ";
	if(trim($directorio," /")=="webmail") $retorno = $escribir['dir_no_perm']." ";
	return $retorno;
}

function directorio_vacio($directorio_oscommerce,$directorio_raiz)
{
global $escribir;
$retorno = NULL;
$archivos = NULL;

if(is_file(rtrim($directorio_oscommerce,"/ "))) $retorno = $escribir['direc_es_archivo'];

if($directorio_oscommerce==$directorio_raiz)
	{
	$archivos = scandir($directorio_oscommerce);
	...
	{
	if(is_dir($directorio_oscommerce))
		{
		$archivos = scandir($directorio_oscommerce);
		if(is_array($archivos))
			{
...
function agregar_sql_oscommerce($base_mysql,$usuario_mysql,$password_mysql)
{
	global $nombre_servidor;
	global $ip_db_servidor;
	$checkeo = true;
	...
	$servidor_db_host = $ip_db_servidor[$nombre_servidor];
	$archivo_sql = file_get_contents("/usr/share/gnupanel/usuarios/plugins/herramientas/oscommerce/gnupanel-oscommerce.sql");
	...
	if(is_array($sqls))
	{
		...
		if(!$checkeo) print "$consulta<br><br>\n";
		}
...
function modificar_subdominio($id_apache,$php_register_globals,$php_safe_mode,$indexar)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
	...
	$conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
	$conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
	$consulta = "UPDATE gnupanel_apacheconf SET php_register_globals = $php_register_globals, php_safe_mode = $php_safe_mode, indexar = $indexar, estado = 1 WHERE id_apache = $id_apache AND id_dominio = $id_usuario ";
	$res_consulta = pg_query($conexion,$consulta);
...
function existe_dir_prot($id_apache,$directorio)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id_directorio FROM gnupanel_apache_dir_prot WHERE id_subdominio = $id_apache AND directorio = '$directorio' ";
    $res_consulta = pg_query($conexion,$consulta);
    $cantidad = pg_num_rows($res_consulta);
    if($cantidad > 0) $result = true;
    pg_close($conexion);
...
function agregar_dir_prot($id_apache,$directorio)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    if(!existe_dir_prot($id_apache,$directorio))
	{
	$descripcion = "osCommerce";
	$pasaporte = gnupanel_crypt($password);
	$conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
	$conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
	$consulta = "INSERT INTO gnupanel_apache_dir_prot(id_subdominio,directorio,descripcion) VALUES($id_apache,'$directorio','$descripcion') ";
	$res_consulta = pg_query($conexion,$consulta);
	}
    else
	{
...
function desproteger_directorio($id_apache,$directorio)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "DELETE FROM gnupanel_apache_dir_prot WHERE id_subdominio = $id_apache AND directorio = '$directorio' ";
    $res_consulta = pg_query($conexion,$consulta);

    return $res_consulta;
}

function oscommerce($id_apache,$directorio,$base_mysql,$usuario_mysql,$password_mysql,$admin_oscommerce,$password_oscommerce)
{
	global $_SESSION;
	global $nombre_servidor;
	global $ip_db_servidor;
	...
	agregar_dir_prot($id_apache,"/".$directorio."/admin");
	agregar_usuario_apache($id_usuario,$id_apache,$admin_oscommerce,$password_oscommerce);
	modificar_subdominio($id_apache,1,1,1);
	$dominio_oscommerce = "";

	if(strlen($subdominio_data['subdominio'])==0)
	{
		$dominio_oscommerce = $dominio_oscommerce.$dominio;
	}
	else
	{
		$dominio_oscommerce = $dominio_oscommerce.$subdominio_data['subdominio'].".".$dominio;
	}

	$checkeo = agregar_base_datos_my($id_usuario,$base_mysql,$usuario_mysql,$password_mysql,$tipo);
	$checkeo = $checkeo && agregar_sql_oscommerce($base_mysql,$usuario_mysql,$password_mysql);
	
	$es_ssl = "NO";
	if($subdominio_data['es_ssl']==1) $es_ssl = "SI";

	$directorio_raiz = pone_barra($subdominio_data['documentroot']);
	$directorio_oscommerce = pone_barra(pone_barra($subdominio_data['documentroot']).$directorio);
	$directorio_ruta = "/".pone_barra($directorio);
	$comando = "/usr/local/gnupanel/bin/oscommerce-inst.sh $directorio_oscommerce $directorio_raiz $dominio_oscommerce $directorio_ruta $es_ssl $host_mysql $base_mysql $usuario_mysql $password_mysql ";
	
	$sistema = system($comando);

	if(!$checkeo)
	{
		borra_usuario_apache($admin_oscommerce);
		desproteger_directorio($id_apache,"/".$directorio."/admin");
		modificar_subdominio($id_apache,0,1,0);
		borrar_base_datos_my($base_mysql,$usuario_mysql);
	}
	...
	$checkeo = $prefijo.$dominio_oscommerce.pone_barra_principio($directorio);
	}
...
function oscommerce_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	print "<td width=\"30%\" > \n";
	$escriba = $escribir['inst_oscommerce'];
...
function oscommerce_1($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$admin_oscommerce = $_POST['admin_oscommerce'];
	$password_oscommerce = $_POST['password_oscommerce'];
	$password_oscommerce_1 = $_POST['password_oscommerce_1'];
	...
	genera_fila_formulario("admin_oscommerce",$admin_oscommerce,"text_con_text",18,!$mensaje,true,NULL,254,"@".$dominio);
	genera_fila_formulario("password_oscommerce",$password_oscommerce,'password',NULL,!$mensaje);
	genera_fila_formulario("password_oscommerce_1",$password_oscommerce_1,'password',NULL,!$mensaje);
	...
	genera_fila_formulario("inst_oscommerce",NULL,'submit',NULL,true);
	print "</table> \n";
	print "</form> \n";
	print "</ins> \n";
	print "</div> \n";
...
function oscommerce_2($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
;
	...
	$checkeo = verifica_oscommerce($directorio,$base_mysql,$usuario_mysql,$password_mysql,$admin_oscommerce,$password_oscommerce,$password_oscommerce_1,$dominio,$subdominio_data);
	
	if($checkeo)
	{
	oscommerce_1($procesador,$checkeo);
	}
	else
	{
	$directorio_raiz = pone_barra($subdominio_data['documentroot']);
	...
	$cuidado = directorio_vacio($directorio_oscommerce,$directorio_raiz);
	print "<div id=\"formulario\" > \n";
	print "<ins> \n";
	print "<form method=\"post\" action=\"$procesador&#063;seccion&#061;$seccion&#038;plugin&#061;$plugin\" > \n";
	print "<table width=\"80%\" > \n";
	...
	genera_fila_formulario("admin_oscommerce",$admin_oscommerce,'hidden',NULL,true);
	genera_fila_formulario("password_oscommerce",$password_oscommerce,'hidden',NULL,true);
	genera_fila_formulario("password_oscommerce_1",$password_oscommerce_1,'hidden',NULL,true);
...
function oscommerce_3($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$checkeo = oscommerce($id_apache,$directorio,$base_mysql,$usuario_mysql,$password_mysql,$admin_oscommerce."@".$dominio,$password_oscommerce);
...
function oscommerce_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		oscommerce_1($nombre_script,NULL);
		break;

		case "2":
		oscommerce_2($nombre_script,NULL);
		break;

		case "3":
		oscommerce_3($nombre_script,NULL);
		break;

		default:
		oscommerce_0($nombre_script,NULL);
	}
}
