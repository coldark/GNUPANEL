...
function verifica_smf($directorio,$base_mysql,$usuario_mysql,$password_mysql,$sitio_smf,$correo_smf,$admin_smf,$password_smf,$password_smf_1,$dominio,$subdominio_data,$licencia)
{
	global $escribir;
	$retorno = NULL;
	$tipo = 1;
	...
	if(!verifica_correo($correo_smf)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($admin_smf,NULL)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($sitio_smf,NULL,true,true)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($password_smf,NULL)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($password_smf_1,NULL)) $retorno = $escribir['carac_inv']." ";
	...
	if($licencia != "true") $retorno = $escribir['no_licencia']." ";
	if($password_smf != $password_smf_1) $retorno = $escribir['distintos']." ";
	if(strlen($password_smf) < 8) $retorno = $escribir['pocos_carac']." ";
	$directorio_smf = pone_barra(pone_barra($subdominio_data['documentroot']).$directorio);
	$archivo_existe_smf = $directorio_smf."gnupanel-smf.php";
	if(is_file($archivo_existe_smf)) $retorno = $escribir['ya_hay_smf']." ";
	if(trim($directorio," /")=="gnupanel") $retorno = $escribir['dir_no_perm']." ";
	if(trim($directorio," /")=="webmail") $retorno = $escribir['dir_no_perm']." ";
	return $retorno;
}

function directorio_vacio($directorio_smf,$directorio_raiz)
{
global $escribir;
$retorno = NULL;
$archivos = NULL;

if(is_file(rtrim($directorio_smf,"/ "))) $retorno = $escribir['direc_es_archivo'];

if($directorio_smf==$directorio_raiz)
	{
	$archivos = scandir($directorio_smf);
...
function agregar_sql_smf($base_mysql,$usuario_mysql,$password_mysql,$password_smf,$correo_smf,$admin_smf,$directorio_smf,$dominio_smf)
{
	global $nombre_servidor;
	global $ip_db_servidor;
	$checkeo = true;
	...
	$servidor_db_host = $ip_db_servidor[$nombre_servidor];
	$archivo_sql = file_get_contents("/usr/share/gnupanel/usuarios/plugins/herramientas/smf/gnupanel-smf.sql");
	...
	if(is_array($sqls))
	{
		...
		if(strlen($consulta)>0)
			{
			$resultado_consulta = mysql_db_query($base_mysql,$consulta,$conexion_mysql);
 			$checkeo = $checkeo && $resultado_consulta;
			}
		}
		
		$tiempo = time();
		$password_smf_crypt = sha1(strtolower($admin_smf.$password_smf));
		...
		$consulta = "UPDATE smf_smileys SET code = ';)' WHERE code = 'PRIMER' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$consulta = "UPDATE smf_smileys SET code = ';D' WHERE code = 'SEGUND' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "attachmentUploadDir";
		$valor = $directorio_smf."attachments";
		$consulta = "UPDATE smf_settings SET value = '$valor' WHERE variable = '$variable' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "smileys_dir";
		$valor = $directorio_smf."Smileys";
		$consulta = "UPDATE smf_settings SET value = '$valor' WHERE variable = '$variable' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "smileys_url";
		$valor = $dominio_smf."Smileys";
		$consulta = "UPDATE smf_settings SET value = '$valor' WHERE variable = '$variable' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "avatar_directory";
		$valor = $directorio_smf."avatars";
		$consulta = "UPDATE smf_settings SET value = '$valor' WHERE variable = '$variable' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "avatar_url";
		$valor = $dominio_smf."avatars";
		$consulta = "UPDATE smf_settings SET value = '$valor' WHERE variable = '$variable' ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "theme_url";
		$valor = $dominio_smf."Themes/default";
		$consulta = "UPDATE smf_themes SET value = '$valor' WHERE variable = '$variable' AND ID_THEME = 1 ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "images_url";
		$valor = $dominio_smf."Themes/default/images";
		$consulta = "UPDATE smf_themes SET value = '$valor' WHERE variable = '$variable' AND ID_THEME = 1 ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "theme_dir";
		$valor = $directorio_smf."Themes/default";
		$consulta = "UPDATE smf_themes SET value = '$valor' WHERE variable = '$variable' AND ID_THEME = 1 ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "theme_url";
		$valor = $dominio_smf."Themes/classic";
		$consulta = "UPDATE smf_themes SET value = '$valor' WHERE variable = '$variable' AND ID_THEME = 2 ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "images_url";
		$valor = $dominio_smf."Themes/classic/images";
		$consulta = "UPDATE smf_themes SET value = '$valor' WHERE variable = '$variable' AND ID_THEME = 2 ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "theme_dir";
		$valor = $directorio_smf."Themes/classic";
		$consulta = "UPDATE smf_themes SET value = '$valor' WHERE variable = '$variable' AND ID_THEME = 2 ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "theme_url";
		$valor = $dominio_smf."Themes/babylon";
		$consulta = "UPDATE smf_themes SET value = '$valor' WHERE variable = '$variable' AND ID_THEME = 3 ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "images_url";
		$valor = $dominio_smf."Themes/babylon/images";
		$consulta = "UPDATE smf_themes SET value = '$valor' WHERE variable = '$variable' AND ID_THEME = 3 ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$variable = "theme_dir";
		$valor = $directorio_smf."Themes/babylon";
		$consulta = "UPDATE smf_themes SET value = '$valor' WHERE variable = '$variable' AND ID_THEME = 3 ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);

		$consulta = "INSERT INTO smf_members (memberName, realName, passwd, emailAddress, ID_GROUP, posts, dateRegistered, hideEmail,passwordSalt, lngfile, personalText, avatar, memberIP, memberIP2, buddy_list, pm_ignore_list, messageLabels, websiteTitle,websiteUrl, location, ICQ, MSN, signature, usertitle, secretQuestion, additionalGroups) VALUES ('$admin_smf','$admin_smf','$password_smf_crypt','$correo_smf', 1, '0',$tiempo, '0', '$salt', '', '', '','', '', '', '', '', '', '', '', '', '', '', '', '', '') ";
		$checkeo = $checkeo && mysql_db_query($base_mysql,$consulta,$conexion_mysql);
...
function smf($id_apache,$directorio,$base_mysql,$usuario_mysql,$password_mysql,$sitio_smf,$correo_smf,$admin_smf,$password_smf)
{
	global $_SESSION;
	global $nombre_servidor;
	global $ip_db_servidor;
	...
	$idioma_smf = "english";
	if($idioma=="es") $idioma_smf = "spanish_es-utf8";
	...
	{
		$dominio_smf = $subdominio_data['subdominio'].".".$dominio;
	}

	$prefijo = "http://";
	if($subdominio_data['es_ssl']==1) $prefijo = "https://";
	$dominio_smf = $prefijo.$dominio_smf.pone_barra_principio($directorio);

	$directorio_raiz = pone_barra($subdominio_data['documentroot']);
	$directorio_smf = rtrim(pone_barra(pone_barra($subdominio_data['documentroot']).$directorio),"/ ");

	$checkeo = agregar_base_datos_my($id_usuario,$base_mysql,$usuario_mysql,$password_mysql,$tipo);
	$checkeo = $checkeo && agregar_sql_smf($base_mysql,$usuario_mysql,$password_mysql,$password_smf,$correo_smf,$admin_smf,pone_barra($directorio_smf),pone_barra($dominio_smf));

	$comando = "/usr/local/gnupanel/bin/smf-inst.sh $directorio_smf $directorio_raiz $host_mysql $base_mysql $usuario_mysql $password_mysql $dominio_smf $idioma_smf $correo_smf \"$sitio_smf\" ";
	
	$sistema = system($comando);
	if(!$checkeo) borrar_base_datos_my($base_mysql,$usuario_mysql);

	if($checkeo) $checkeo = $dominio_smf;
...
function smf_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function smf_1($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$sitio_smf = $_POST['sitio_smf'];
	$correo_smf = $_POST['correo_smf'];
	$admin_smf = $_POST['admin_smf'];
	$password_smf = $_POST['password_smf'];
	$password_smf_1 = $_POST['password_smf_1'];
	...
	genera_fila_formulario("sitio_smf",$sitio_smf,'text',NULL,!$mensaje,true,true);
	genera_fila_formulario("correo_smf",$correo_smf,'text_correo',NULL,!$mensaje);
	genera_fila_formulario("admin_smf",$admin_smf,'text',24,!$mensaje,NULL,NULL,24);
	genera_fila_formulario("password_smf",$password_smf,'password',24,!$mensaje,NULL,NULL,24);
	genera_fila_formulario("password_smf_1",$password_smf_1,'password',24,!$mensaje,NULL,NULL,24);
	genera_fila_formulario(NULL,NULL,"espacio",8,!$mensaje);

	print "<tr> \n";
	print "<td colspan=\"2\" > \n";
	$escriba = $escribir['licencia0'];
	print "$escriba";
	print "</td> \n";
	print "</tr> \n";
	genera_fila_formulario(NULL,NULL,"espacio",8,!$mensaje);

	genera_fila_formulario("licencia",$licencia,'check_box',24,!$mensaje,NULL,NULL,254,$escribir['licencia1']);
	...
	genera_fila_formulario("inst_smf",NULL,'submit',NULL,true);
...
function smf_2($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
;
	...
	$checkeo = verifica_smf($directorio,$base_mysql,$usuario_mysql,$password_mysql,$sitio_smf,$correo_smf,$admin_smf,$password_smf,$password_smf_1,$dominio,$subdominio_data,$licencia);
	
	if($checkeo)
	{
	smf_1($procesador,$checkeo);
	}
	else
	{
	$directorio_raiz = pone_barra($subdominio_data['documentroot']);
	...
	$cuidado = directorio_vacio($directorio_smf,$directorio_raiz);
	print "<div id=\"formulario\" > \n";
	print "<ins> \n";
	print "<form method=\"post\" action=\"$procesador&#063;seccion&#061;$seccion&#038;plugin&#061;$plugin\" > \n";
	print "<table width=\"80%\" > \n";
	...
	genera_fila_formulario("sitio_smf",$sitio_smf,'hidden',NULL,true);
	genera_fila_formulario("correo_smf",$correo_smf,'hidden',NULL,true);
	genera_fila_formulario("admin_smf",$admin_smf,'hidden',NULL,true);
	genera_fila_formulario("password_smf",$password_smf,'hidden',NULL,true);
	genera_fila_formulario("password_smf_1",$password_smf_1,'hidden',NULL,true);
...
function smf_3($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$checkeo = smf($id_apache,$directorio,$base_mysql,$usuario_mysql,$password_mysql,$sitio_smf,$correo_smf,$admin_smf,$password_smf);

	$escriba = NULL;
	if($checkeo)
	{
		$acceder = $checkeo;
		$escriba = $escribir['exito']."<br><br>".$escribir['acceder']."<a href=\"$acceder\" target=\"_blank\" >".$acceder."</a><br><br>";
	}
	else
	{
...
function smf_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		smf_1($nombre_script,NULL);
		break;

		case "2":
		smf_2($nombre_script,NULL);
		break;

		case "3":
		smf_3($nombre_script,NULL);
		break;

		default:
		smf_0($nombre_script,NULL);
	}
}
