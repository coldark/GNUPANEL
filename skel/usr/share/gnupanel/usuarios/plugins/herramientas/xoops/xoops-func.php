...
function verifica_xoops($directorio,$base_mysql,$usuario_mysql,$password_mysql,$correo_xoops,$usuario_xoops,$password_xoops,$password_xoops_1,$dominio,$subdominio_data)
{
	global $escribir;
	$retorno = NULL;
	$tipo = 1;
	...
	if(!verifica_dato($usuario_xoops,NULL)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($password_xoops,NULL)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_dato($password_xoops_1,NULL)) $retorno = $escribir['carac_inv']." ";
	if(!verifica_correo($correo_xoops)) $retorno = $escribir['carac_inv']." ";
	...
	if($password_xoops != $password_xoops_1) $retorno = $escribir['distintos']." ";
	if(strlen($password_xoops) < 8) $retorno = $escribir['pocos_carac']." ";
	$directorio_xoops = pone_barra(pone_barra($subdominio_data['documentroot']).$directorio);
	$archivo_existe_xoops = $directorio_xoops."gnupanel-xoops.php";
	if(is_file($archivo_existe_xoops)) $retorno = $escribir['ya_hay_xoops']." ";
	if(trim($directorio," /")=="gnupanel") $retorno = $escribir['dir_no_perm']." ";
	if(trim($directorio," /")=="webmail") $retorno = $escribir['dir_no_perm']." ";
	return $retorno;
}

function directorio_vacio($directorio_xoops,$directorio_raiz)
{
global $escribir;
$retorno = NULL;
$archivos = NULL;

if(is_file(rtrim($directorio_xoops,"/ "))) $retorno = $escribir['direc_es_archivo'];

if($directorio_xoops==$directorio_raiz)
	{
	$archivos = scandir($directorio_xoops);
...
function agregar_sql_xoops($base_mysql,$usuario_mysql,$password_mysql,$usuario_xoops,$password_xoops,$correo_xoops,$dominio_xoops)
{
	global $nombre_servidor;
	global $ip_db_servidor;
	global $idioma;
	...
	$servidor_db_host = $ip_db_servidor[$nombre_servidor];
	$idioma_xoops = "english";
	if($idioma == "es") $idioma_xoops = "spanish";
	$archivo_sql = file_get_contents("/usr/share/gnupanel/usuarios/plugins/herramientas/xoops/gnupanel-xoops.sql");
	...
	if(is_array($sqls))
	{
		...
		$consulta = trim(str_replace("DOMINIO_XOOPS_CON_DIR",$dominio_xoops,$consulta));
		$consulta = trim(str_replace("IDIOMA_XOOPS",$idioma_xoops,$consulta));
		$consulta = trim(str_replace("TIEMPO_XOOPS",$tiempo_xoops,$consulta));
		$consulta = trim(str_replace("USUARIO_XOOPS",$usuario_xoops,$consulta));
		$consulta = trim(str_replace("CORREO_XOOPS",$correo_xoops,$consulta));
		$consulta = trim(str_replace("PASAPORTE_XOOPS_MD5",$password_xoops,$consulta));
		$res_consulta = mysql_db_query($base_mysql,$consulta,$conexion_mysql);
		if(strlen($consulta)>0) $checkeo = $checkeo && $res_consulta;
		}
...
function xoops($id_apache,$directorio,$base_mysql,$usuario_mysql,$password_mysql,$correo_xoops,$usuario_xoops,$password_xoops)
{
	global $_SESSION;
	global $nombre_servidor;
	global $ip_db_servidor;
	...
	$id_usuario = $_SESSION['id_usuario'];
	$tipo = 1;
	$password_xoops_crypt = md5($password_xoops);
	...
	$idioma_xoops = "en";
	if($idioma = "es") $idioma_xoops = "es";
	
	$dominio_xoops = "http://";
	if($subdominio_data['es_ssl']==1) $dominio_xoops = "https://";

	if(strlen($subdominio_data['subdominio'])==0)
	{
		$dominio_xoops = $dominio_xoops.$dominio."/".$directorio;
	}
	else
	{
		$dominio_xoops = $dominio_xoops.$subdominio_data['subdominio'].".".$dominio."/".$directorio;
	}

	$checkeo = $checkeo && agregar_sql_xoops($base_mysql,$usuario_mysql,$password_mysql,$usuario_xoops,$password_xoops_crypt,$correo_xoops,$dominio_xoops);

	$directorio_raiz = pone_barra($subdominio_data['documentroot']);
	$directorio_xoops = pone_barra(pone_barra($subdominio_data['documentroot']).$directorio);
	$comando = "/usr/local/gnupanel/bin/xoops-inst.sh $directorio_xoops $dominio_xoops $host_mysql $usuario_mysql $password_mysql $base_mysql $idioma_xoops $correo_xoops $directorio_raiz ";
	
	$sistema = system($comando);
	if(!$checkeo) borrar_base_datos_my($base_mysql,$usuario_mysql);
	
	if($checkeo) $checkeo = $checkeo && cambia_caracteres_seteo($id_apache);
	if($checkeo) $checkeo = $dominio_xoops;
...
function cambia_caracteres_seteo($id_apache)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $retorno = NULL;
    $caracteres = "UTF-8";
    if($idioma == "es") $caracteres = "ISO-8859-1";

    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "UPDATE gnupanel_apacheconf SET estado = 1, caracteres = '$caracteres' WHERE id_apache = $id_apache ";
    $res_consulta = pg_query($conexion,$consulta);
...
function xoops_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function xoops_1($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$correo_xoops = $_POST['correo_xoops'];
	$usuario_xoops = $_POST['usuario_xoops'];
	$password_xoops = $_POST['password_xoops'];
	$password_xoops_1 = $_POST['password_xoops_1'];
	...
	genera_fila_formulario("correo_xoops",$correo_xoops,'text_correo',NULL,!$mensaje);
	genera_fila_formulario("usuario_xoops",$usuario_xoops,'text',13,!$mensaje,NULL);
	genera_fila_formulario("password_xoops",$password_xoops,'password',NULL,!$mensaje);
	genera_fila_formulario("password_xoops_1",$password_xoops_1,'password',NULL,!$mensaje);
	...
	genera_fila_formulario("inst_xoops",NULL,'submit',NULL,true);
...
function xoops_2($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
;
	...
	$checkeo = verifica_xoops($directorio,$base_mysql,$usuario_mysql,$password_mysql,$correo_xoops,$usuario_xoops,$password_xoops,$password_xoops_1,$dominio,$subdominio_data);
	
	if($checkeo)
	{
	xoops_1($procesador,$checkeo);
	}
	else
	{
	$directorio_raiz = pone_barra($subdominio_data['documentroot']);
	...
	$cuidado = directorio_vacio($directorio_xoops,$directorio_raiz);
	print "<div id=\"formulario\" > \n";
	print "<ins> \n";
	print "<form method=\"post\" action=\"$procesador&#063;seccion&#061;$seccion&#038;plugin&#061;$plugin\" > \n";
	print "<table width=\"80%\" > \n";
	...
	genera_fila_formulario("correo_xoops",$correo_xoops,'hidden',NULL,true);
	genera_fila_formulario("usuario_xoops",$usuario_xoops,'hidden',NULL,true);
	genera_fila_formulario("password_xoops",$password_xoops,'hidden',NULL,true);
	genera_fila_formulario("password_xoops_1",$password_xoops_1,'hidden',NULL,true);
...
function xoops_3($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$checkeo = xoops($id_apache,$directorio,$base_mysql,$usuario_mysql,$password_mysql,$correo_xoops,$usuario_xoops,$password_xoops);

	$escriba = NULL;
	if($checkeo)
	{
		$acceder = $checkeo;
		$escriba = $escribir['exito']."<br><br>".$escribir['acceder']." <a href=\"$acceder\" target=\"_blank\" >".$acceder."</a><br>";
	}
	else
	{
...
function xoops_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		xoops_1($nombre_script,NULL);
		break;

		case "2":
		xoops_2($nombre_script,NULL);
		break;

		case "3":
		xoops_3($nombre_script,NULL);
		break;

		default:
		xoops_0($nombre_script,NULL);
	}
}
