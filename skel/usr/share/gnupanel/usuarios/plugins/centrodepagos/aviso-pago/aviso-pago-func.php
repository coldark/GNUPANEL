...
function aviso_pago($monto,$codigo_seguimiento)
{
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "INSERT INTO pagos_usuario(id_usuario,importe,empresa_cobranza,codigo_seguimiento) VALUES($id_usuario,$monto,'aviso_pago','$codigo_seguimiento') ";
    $res_consulta = pg_query($conexion,$consulta);
    $result = $res_consulta;
pg_close($conexion);
return $result;
...
function aviso_pago_0($procesador,$mensaje)
{
	...
	if(!es_usuario_principal($_SESSION['id_usuario']))
	{
