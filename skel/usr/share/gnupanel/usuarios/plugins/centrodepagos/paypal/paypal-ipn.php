...
function dame_link_paypal($id_usuario)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $consulta = "SELECT test FROM paypal_reseller WHERE id_reseller = (SELECT cliente_de FROM gnupanel_usuario WHERE id_usuario = $id_usuario) ";
    $res_consulta = pg_query($conexion,$consulta);
...
function dame_id_pago_usuario($control,$empresa)
{
    ...
    $consulta = "SELECT id,id_usuario FROM pagos_usuario WHERE codigo_seguimiento = '$control' AND empresa_cobranza = '$empresa' AND confirmado = 0 AND acreditado = 0";
...
$data_usuario = dame_id_pago_usuario(trim($data_pago['codigo_seguimiento']),"paypal");

$link_paypal = dame_link_paypal($data_usuario['id_usuario']);
...
else
{
	...
	while (!feof($fp))
	{
		...
		{
			if($data_usuario['id']==$data_pago['id_pago'])
			{
				acreditar_fondos_usuario($data_usuario['id_usuario'],$data_pago['id_pago'],$data_pago['importe']);
			}
		}
