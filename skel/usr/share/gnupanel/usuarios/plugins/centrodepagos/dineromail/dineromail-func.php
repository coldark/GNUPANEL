...
function dineromail_configurado()
{
    ...
    $consulta = "SELECT * FROM dineromail_reseller WHERE id_reseller = (SELECT cliente_de FROM gnupanel_usuario WHERE id_usuario = $id_usuario) ";
    $res_consulta = pg_query($conexion,$consulta);
...
function dineromail($monto)
{
    ...
    $consulta = "INSERT INTO pagos_usuario(id_usuario,importe,empresa_cobranza,codigo_seguimiento) VALUES($id_usuario,$monto,'dineromail','$codigo_seguimiento') ";
    $res_consulta = pg_query($conexion,$consulta);
...
function dineromail_1($nombre_script,$mensaje)
{
	...
	$variables['NombreItem'] = "U_".dame_usuario();
