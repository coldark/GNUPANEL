...
function redireccionar_usuario_correo_0($procesador,$mensaje)
{
	...
	print "</td> \n";
	print "<td width=\"40%\" > \n";
	$escriba = $escribir['redireccionar'];
...
function redireccionar_usuario_correo_2($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	if($checkeo)
	{
	redireccionar_usuario_correo_1($procesador,$checkeo);
	}
	else
	{
	$escriba = "";
...
function redireccionar_usuario_correo_init($nombre_script)
{
	...
	switch($paso)
	{
		...
		case "2":
		redireccionar_usuario_correo_2($nombre_script,NULL);
		break;
