...
function cambia_quota_correo($address,$quota)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db ";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "UPDATE gnupanel_postfix_mailuser SET mailquota = $quota WHERE address = '$address' AND id_dominio = $id_usuario ";
    $res_consulta = pg_query($conexion,$consulta);
...
function dame_quota_actual($address)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db ";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT mailquota FROM gnupanel_postfix_mailuser WHERE address = '$address' AND id_dominio = $id_usuario ";
    $res_consulta = pg_query($conexion,$consulta);
...
function verifica_cambia_quota_correo($quota)
{
	global $escribir;
	$retorno = NULL;
...
function cambia_quota_correo_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function cambia_quota_correo_1($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$quota = dame_quota_actual($address);
	if(isset($_POST['quota'])) $quota = $_POST['quota'];
	print "<div id=\"formulario\" > \n";
...
function cambia_quota_correo_2($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$comienzo = $_POST['comienzo'];
	$usuario_data = $address;
	$checkeo = verifica_cambia_quota_correo($quota);

	if($checkeo)
	{
	cambia_quota_correo_1($procesador,$checkeo);
	}
	else
	{
	$escriba = "";
	if(cambia_quota_correo($address,$quota))
		{
...
function cambia_quota_correo_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		cambia_quota_correo_1($nombre_script,NULL);
		break;

		case "2":
		cambia_quota_correo_2($nombre_script,NULL);
		break;

		default:
		cambia_quota_correo_0($nombre_script,NULL);
	}
}
