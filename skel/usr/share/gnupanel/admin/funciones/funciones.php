...
function dame_id_admin($admin)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id_admin FROM gnupanel_admin WHERE admin='$admin'";
    $res_consulta = pg_query($conexion,$consulta);
...
function dame_id_reseller($reseller)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id_reseller FROM gnupanel_reseller WHERE reseller='$reseller'";
    $res_consulta = pg_query($conexion,$consulta);
    $retorno = pg_fetch_result ($res_consulta,0,0);
    pg_free_result($res_consulta);
    pg_close($conexion);
...
function checkea_pasaporte_admin($admin,$pasaporte)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT * from gnupanel_admin WHERE admin='$admin' ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function dame_idioma_admin($id_admin)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT idioma FROM gnupanel_admin_lang WHERE id_admin = $id_admin ";
    $res_consulta = pg_query($conexion,$consulta);
    $retorno = pg_fetch_result ($res_consulta,0,0);
    pg_free_result($res_consulta);
    pg_close($conexion);
...
function dame_tema_interfaz_admin()
{
    global $_SERVER;
    global $_SESSION;
    ...
    $result = "estilos/".dame_tema()."/estilos";
...
function dame_tema_interfaz_admin_per()
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    if($logueado==1)
	{
		...
		$consulta = "SELECT cliente_de from gnupanel_reseller WHERE dominio = '$dominio' " ;
		$res_consulta = pg_query($conexion,$consulta);
		$checkeo = $res_consulta;

		if(pg_num_rows ($res_consulta)>0) $id_admin = pg_fetch_result($res_consulta,0,0);
		if($id_admin)
		{
			$consulta = "SELECT id_tema from gnupanel_admin_sets WHERE id_admin = $id_admin " ;
			$res_consulta = pg_query($conexion,$consulta);
			$checkeo = $checkeo && $res_consulta;
...
function dame_tema()
{
    ...
    $consulta = "SELECT tema from gnupanel_temas WHERE id_tema = (SELECT id_tema from gnupanel_admin_sets WHERE id_admin = $id_admin) " ;
    $res_consulta = pg_query($conexion,$consulta);
...
function dame_planes_reseller()
{
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT DISTINCT plan from gnupanel_reseller_planes WHERE id_dueno=$id_admin ORDER BY plan";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function dame_vigencias_plan($plan)
{
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT vigencia from gnupanel_reseller_planes WHERE id_dueno=$id_admin AND plan = '$plan' ORDER BY vigencia ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function corresponde_vigencia($plan,$vigencia)
{
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT count(id_plan) from gnupanel_reseller_planes WHERE id_dueno = $id_admin AND plan = '$plan' AND vigencia = $vigencia ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function dame_precio_plan($plan,$vigencia)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT precio,moneda from gnupanel_reseller_planes WHERE id_dueno=$id_admin AND plan = '$plan' AND vigencia = $vigencia ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function dame_tiempo_max_sesion_admin($id_admin)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT tiempo_max_sesion FROM gnupanel_admin_sets WHERE id_admin = $id_admin ";
    $res_consulta = pg_query($conexion,$consulta);
    $retorno = pg_fetch_result ($res_consulta,0,0);
    pg_free_result($res_consulta);
    pg_close($conexion);
...
function dame_cant_max_result_admin($id_admin)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT cant_max_result FROM gnupanel_admin_sets WHERE id_admin = $id_admin ";
    $res_consulta = pg_query($conexion,$consulta);
    $retorno = pg_fetch_result ($res_consulta,0,0);
    pg_free_result($res_consulta);
    pg_close($conexion);
...
function dame_dias_de_gracia_admin($id_admin)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT dias_de_gracia FROM gnupanel_admin_sets WHERE id_admin = $id_admin ";
    $res_consulta = pg_query($conexion,$consulta);
    $retorno = pg_fetch_result ($res_consulta,0,0);
    pg_free_result($res_consulta);
    pg_close($conexion);
