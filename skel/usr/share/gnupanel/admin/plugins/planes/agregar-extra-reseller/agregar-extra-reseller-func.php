...
function agregar_extra_reseller($periodo,$cantidad,$servicio,$precio,$id_moneda)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $consulta = "INSERT INTO gnupanel_reseller_precios_extras(periodo,id_dueno,cantidad,servicio,precio,id_moneda) VALUES ($periodo,$id_admin,$cantidad,'$servicio',$precio,$id_moneda) ";
    $res_consulta = pg_query($conexion,$consulta);
    pg_close($conexion);
    return $res_consulta;
    }

function verifica_agregar_extra_reseller($periodo,$cantidad,$servicio,$precio,$id_moneda)
{
global $escribir;
$retorno = NULL;
...
if(existe_extra_reseller($periodo,$cantidad,$servicio,$id_moneda)) return $escribir['ya_existe']." ";
return $retorno;
}

function existe_extra_reseller($periodo,$cantidad,$servicio,$id_moneda)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $consulta = "SELECT * FROM gnupanel_reseller_precios_extras WHERE id_dueno = $id_admin AND periodo = $periodo AND cantidad = $cantidad AND servicio = '$servicio' AND id_moneda = $id_moneda ";
    $res_consulta = pg_query($conexion,$consulta);
...
function agregar_extra_reseller_0($procesador,$mensaje)
{
    global $idioma;
    global $escribir;
    global $plugin;
...
function agregar_extra_reseller_1($procesador,$mensaje)
{
    global $idioma;
    global $escribir;
    global $plugin;
    ...
    $checkea = verifica_agregar_extra_reseller($periodo,$cantidad,$servicio,$precio,$id_moneda);

	if($checkea)
	{
	agregar_extra_reseller_0($procesador,$checkea);
	}
	else
	{
	print "<div id=\"formulario\" > \n";
	$chequeo = agregar_extra_reseller($periodo,$cantidad,$servicio,$precio,$id_moneda);
	if($chequeo)
		{
...
function agregar_extra_reseller_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		agregar_extra_reseller_1($nombre_script,NULL);
		break;

		default:
		agregar_extra_reseller_0($nombre_script,NULL);
	}
}
