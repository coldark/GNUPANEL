...
function dame_plan_reseller($plan,$vigencia)
    ...
    $consulta = "SELECT moneda from gnupanel_monedas WHERE id_moneda = ".$retorno['moneda'];
    $res_consulta = pg_query($conexion,$consulta);
    $moneda = pg_fetch_result($res_consulta,0,0);
    $retorno['moneda'] = $moneda;

pg_close($conexion);
return $retorno;    
}

function listar_plan_reseller_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function listar_plan_reseller_1($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function listar_plan_reseller_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		listar_plan_reseller_1($nombre_script,NULL);
		break;
		default:
		listar_plan_reseller_0($nombre_script,NULL);
	}
}
