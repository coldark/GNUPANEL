...
function cantidad_tickets_sin_responder()
    ...
    $consulta = "SELECT * from gnupanel_tickets_reseller WHERE id_cliente_de = $id_admin AND atendido = 0 ";
...
function lista_tickets_sin_responder($comienzo)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id,id_ticket,id_reseller,asunto FROM gnupanel_tickets_reseller WHERE id_cliente_de = $id_admin AND atendido = 0 ORDER BY id LIMIT $cant_max_result OFFSET $comienzo";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function responder_ticket($id,$respuesta_in)
{
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db ";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "UPDATE gnupanel_tickets_reseller SET texto_r = '$respuesta', atendido = 1 WHERE id = $id ";
    $res_consulta = pg_query($conexion,$consulta);
