...
function cantidad_tickets_respondidos()
    ...
    $consulta = "SELECT * from gnupanel_tickets_reseller WHERE id_cliente_de = $id_admin AND atendido = 1 AND conforme = 1 ";
...
function lista_tickets_respondidos($comienzo)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id,id_ticket,id_reseller,asunto FROM gnupanel_tickets_reseller WHERE id_cliente_de = $id_admin AND atendido = 1 AND conforme = 1 ORDER BY id LIMIT $cant_max_result OFFSET $comienzo";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function dame_ticket_respondido($id)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id,id_ticket,id_reseller,generado,asunto,texto_p,texto_r FROM gnupanel_tickets_reseller WHERE id = $id ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	return NULL;
...
function historial_tickets_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	print "<td width=\"30%\" > \n";
	$escriba = dame_usuario($arreglo['id_reseller']);
	print "$escriba \n";
	print "</td> \n";
...
function historial_tickets_1($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	print "<table width=\"90%\" > \n";
		...
		print "</td> \n";
		print "<td> \n";
		$escriba = dame_usuario($ticket_data['id_reseller']);
		print "$escriba \n";
		print "</td> \n";
		print "</tr> \n";
...
function historial_tickets_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		historial_tickets_1($nombre_script,NULL);
		break;

		default:
		historial_tickets_0($nombre_script,NULL);
	}
}
