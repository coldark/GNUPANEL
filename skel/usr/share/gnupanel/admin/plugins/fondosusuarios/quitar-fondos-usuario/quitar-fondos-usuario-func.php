...
function verifica_quitar_fondos_usuario($fondos)
{
    global $escribir;
	$retorno = NULL;
...
function quitar_fondos_usuario($id_reseller,$fondos)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
...
function quitar_fondos_usuario_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$usuarios = lista_usuarios_reseller($comienzo);
	...
	{
	if($id_principal!=$arreglo['id_reseller'])
	{
		print "<tr> \n";
		print "<td width=\"40%\" > \n";
...
function quitar_fondos_usuario_1($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	genera_fila_formulario('fondos',$fondos,'text_int',8,NULL,true,NULL,254,dame_divisa_reseller($id_reseller));
	...
	genera_fila_formulario("quitar",NULL,'submit',NULL,true,NULL);
...
function quitar_fondos_usuario_2($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$checkeo = NULL;
	$comienzo = $_POST['comienzo'];
	$checkeo = verifica_quitar_fondos_usuario($fondos);

	if($checkeo)
	{
	quitar_fondos_usuario_1($nombre_script,$checkeo);
	}
	else
	{
	$escriba = NULL;
	if(quitar_fondos_usuario($id_reseller,$fondos))
	{
...
function quitar_fondos_usuario_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		quitar_fondos_usuario_1($nombre_script,NULL);
		break;
		case "2":
		quitar_fondos_usuario_2($nombre_script,NULL);
		break;
		default:
		quitar_fondos_usuario_0($nombre_script,NULL);
	}
}
