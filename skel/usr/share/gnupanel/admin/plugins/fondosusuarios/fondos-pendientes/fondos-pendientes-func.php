...
function lista_pagos_pendientes_reseller($comienzo)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $consulta = "SELECT id,id_reseller,importe,inicio_pago,empresa_cobranza,codigo_seguimiento FROM pagos_reseller WHERE confirmado = 0 AND acreditado = 0 AND EXISTS (SELECT * FROM gnupanel_reseller WHERE gnupanel_reseller.id_reseller = pagos_reseller.id_reseller AND gnupanel_reseller.cliente_de = $id_admin) ORDER BY id LIMIT $cant_max_result OFFSET $comienzo";
...
function cantidad_pagos_pendientes_reseller()
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id,id_reseller,importe,inicio_pago,empresa_cobranza,codigo_seguimiento FROM pagos_reseller WHERE confirmado = 0 AND acreditado = 0 AND EXISTS (SELECT * FROM gnupanel_reseller WHERE gnupanel_reseller.id_reseller = pagos_reseller.id_reseller AND gnupanel_reseller.cliente_de = $id_admin) ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function dame_reseller($id_reseller)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
...
function acreditar_fondos($id_reseller,$id_pago,$fondos)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
...
function descartar_pago($id_reseller,$id_pago)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $consulta = "DELETE FROM pagos_reseller WHERE id_reseller = $id_reseller AND id = $id_pago ";
    $res_consulta = pg_query($conexion,$consulta);
    $checkeo = $checkeo && $res_consulta;
...
function dame_divisa_reseller($id_reseller)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
...
function fondos_pendientes_0($procesador,$mensaje)
{
	...
	$cantidad = cantidad_pagos_pendientes_reseller();
	$id_principal = dame_reseller_principal();
	if(!isset($comienzo)) $comienzo = 0;
	$deudas = lista_pagos_pendientes_reseller($comienzo);
	...
	{
	if($id_principal!=$arreglo['id_reseller'] || true)
	{
		print "<tr> \n";
		print "<td width=\"25%\" > \n";
		$escriba = dame_reseller($arreglo['id_reseller']);
		print "$escriba \n";
		print "</td> \n";
		print "<td width=\"15%\" > \n";
		...
		print "<td width=\"10%\" > \n";
		$escriba = $arreglo['importe']." ".dame_divisa_reseller($arreglo['id_reseller']);
		print "$escriba \n";
		print "</td> \n";
...
function fondos_pendientes_1($nombre_script,$mensaje)
{
	...
	$escriba = NULL;
	if(acreditar_fondos($id_reseller,$id_pago,$fondos))
	{
...
function fondos_pendientes_2($nombre_script,$mensaje)
{
	...
	$escriba = NULL;
	if(descartar_pago($id_reseller,$id_pago))
	{
