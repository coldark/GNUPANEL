...
function dame_sets_reseller($id_reseller)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT * FROM gnupanel_reseller_sets WHERE id_reseller = $id_reseller";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	return NULL;
...
function verifica_modificar_sets_reseller($tiempo_max_sesion,$cant_max_result)
{
    global $escribir;
	$retorno = NULL;
...
function modificar_sets_reseller($id_reseller,$tiempo_max_sesion,$cant_max_result,$tema)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "UPDATE gnupanel_reseller_sets SET tiempo_max_sesion = $tiempo_max_sesion, cant_max_result = $cant_max_result, id_tema = (SELECT id_tema FROM gnupanel_temas WHERE tema = '$tema') WHERE id_reseller = $id_reseller";
    $res_consulta = pg_query($conexion,$consulta);
...
function modificar_sets_reseller_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function modificar_sets_reseller_1($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$reseller_sets = dame_sets_reseller($id_reseller);
	$tema = nombre_tema($reseller_sets['id_tema']);
	$temas = dame_temas();
	print "<div id=\"formulario\" > \n";
	...
	if(is_array($reseller_sets))
	{
	foreach($reseller_sets as $llave => $arreglo)
	{
	...
	$tipo_form = "select_ip";
	genera_fila_formulario('id_tema',$temas,$tipo_form,NULL,NULL);
	}
...
function modificar_sets_reseller_2($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$checkeo = NULL;
	$comienzo = $_POST['comienzo'];
	$checkeo = verifica_modificar_sets_reseller($tiempo_max_sesion,$cant_max_result);

	if($checkeo)
	{
	modificar_sets_reseller_1($nombre_script,$checkeo);
	}
	else
	{
	$escriba = NULL;
	if(modificar_sets_reseller($id_reseller,$tiempo_max_sesion,$cant_max_result,$tema))
	{
...
function modificar_sets_reseller_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		modificar_sets_reseller_1($nombre_script,NULL);
		break;
		case "2":
		modificar_sets_reseller_2($nombre_script,NULL);
		break;
		default:
		modificar_sets_reseller_0($nombre_script,NULL);
	}
}
