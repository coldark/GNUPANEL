...
function dame_extras_con_vigencia($id_reseller)
{
	global $servidor_db;
	global $puerto_db;
	global $database;
	...
	$consulta = "SELECT * FROM gnupanel_reseller_precios_extras WHERE id_dueno = $id_admin AND EXISTS (SELECT * FROM gnupanel_reseller_extras WHERE id_reseller = $id_reseller AND gnupanel_reseller_extras.id_extra = gnupanel_reseller_precios_extras.id_extra)";
	$res_consulta = pg_query($conexion,$consulta);
...
function cantidad_usuarios_reseller()
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT * from gnupanel_reseller WHERE cliente_de = $id_admin AND EXISTS (SELECT * FROM gnupanel_reseller_extras WHERE gnupanel_reseller_extras.id_reseller = gnupanel_reseller.id_reseller) ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function lista_usuarios_reseller($comienzo)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id_reseller,reseller,dominio FROM gnupanel_reseller WHERE cliente_de = $id_admin AND EXISTS (SELECT * FROM gnupanel_reseller_extras WHERE gnupanel_reseller_extras.id_reseller = gnupanel_reseller.id_reseller) ORDER BY dominio LIMIT $cant_max_result OFFSET $comienzo";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function desasigna_extra_reseller($id_reseller,$id_extra)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $consulta = "DELETE FROM gnupanel_reseller_extras WHERE id_reseller = $id_reseller AND id_extra = $id_extra ";
    $res_consulta = pg_query($conexion,$consulta);
    $checkeo = $checkeo &&  $res_consulta;
...
function desasigna_extra_reseller_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function desasigna_extra_reseller_1($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$extras = NULL;
	$extras = array();
	$extras = dame_extras_con_vigencia($id_reseller);
...
function desasigna_extra_reseller_2($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	if(desasigna_extra_reseller($id_reseller,$id_extra))
	{
...
function desasigna_extra_reseller_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		desasigna_extra_reseller_1($nombre_script,NULL);
		break;
		case "2":
		desasigna_extra_reseller_2($nombre_script,NULL);
		break;
		default:
		desasigna_extra_reseller_0($nombre_script,NULL);
	}
}
