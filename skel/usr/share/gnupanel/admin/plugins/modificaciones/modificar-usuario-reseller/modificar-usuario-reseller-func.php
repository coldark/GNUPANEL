...
function verifica_modificar_usuario_reseller($id_reseller,$reseller,$dominio,$dominio_ant,$correo_contacto)
{
    global $escribir;
	$retorno = NULL;
	...
	if(existe_dominio_reseller($dominio) && ($dominio != $dominio_ant)) $retorno = $escribir['existe']." ";
	return $retorno;
}

function modificar_usuario_reseller($id_reseller,$reseller,$dominio,$correo_contacto)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "UPDATE gnupanel_reseller SET reseller = '$reseller', dominio = '$dominio', correo_contacto = '$correo_contacto' WHERE id_reseller = $id_reseller";
    $res_consulta = pg_query($conexion,$consulta);
...
function modificar_usuario_reseller_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function modificar_usuario_reseller_1($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	{
		if($llave == "correo_contacto")
			{
			$escriba = $escribir[$llave];
			$tipo_form = "text_correo";
			genera_fila_formulario($llave,$arreglo,$tipo_form,40,NULL);
			}
		else
			{
			print "<tr> \n";
			...
			print "</td> \n";
			print "</tr> \n";
			genera_fila_formulario($llave,$arreglo,'hidden',NULL,true,NULL);
			}
	}
	}


	$dominio_ant = $reseller_data['dominio'];


	genera_fila_formulario("id_reseller",$id_reseller,'hidden',NULL,true,NULL);
	genera_fila_formulario("dominio_ant",$dominio_ant,'hidden',NULL,true,NULL);
...
function modificar_usuario_reseller_2($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$dominio_ant = strtolower(trim($_POST['dominio_ant']));
	$checkeo = NULL;
	$comienzo = $_POST['comienzo'];
	$checkeo = verifica_modificar_usuario_reseller($id_reseller,$reseller,$dominio,$dominio_ant,$correo_contacto);

	if($checkeo)
	{
	modificar_usuario_reseller_1($nombre_script,$checkeo);
	}
	else
	{
	$escriba = NULL;
	if(modificar_usuario_reseller($id_reseller,$reseller,$dominio,$correo_contacto))
	{
...
function modificar_usuario_reseller_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		modificar_usuario_reseller_1($nombre_script,NULL);
		break;
		case "2":
		modificar_usuario_reseller_2($nombre_script,NULL);
		break;
		default:
		modificar_usuario_reseller_0($nombre_script,NULL);
	}
}
