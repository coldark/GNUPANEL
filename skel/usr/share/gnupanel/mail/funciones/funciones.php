...
function dame_id_usuario_correo($usuario,$dominio)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id_usuario FROM gnupanel_usuario WHERE dominio='$dominio' ";
    $res_consulta = pg_query($conexion,$consulta);
    $retorno = pg_fetch_result ($res_consulta,0,0);
    pg_free_result($res_consulta);
    pg_close($conexion);
...
function checkea_pasaporte_usuario($usuario,$dominio,$pasaporte)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT passwd from gnupanel_postfix_mailuser WHERE address='$correo' ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
	...
	if($row == 0)
	    ...
	    {
	    $pasaporte_base = pg_fetch_result($res_consulta,0,0);
	    pg_free_result($res_consulta);
	    $semilla = substr($pasaporte_base,0,12);
	    $pasaporte_crypt = crypt($pasaporte,$semilla);
	    if($pasaporte_base==$pasaporte_crypt)
