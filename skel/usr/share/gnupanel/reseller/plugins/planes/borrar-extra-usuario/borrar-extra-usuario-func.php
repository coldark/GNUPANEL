...
function lista_extras_usuario($comienzo)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $consulta = "SELECT * from gnupanel_usuarios_precios_extras WHERE id_dueno = $id_reseller ORDER BY id_extra LIMIT $cant_max_result OFFSET $comienzo";
...
function cantidad_extras_usuario()
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
...
function dame_extra_usuario($id_extra)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT periodo,cantidad,servicio,precio,id_moneda FROM gnupanel_usuarios_precios_extras WHERE id_dueno = $id_reseller AND id_extra = $id_extra ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function verifica_borrar_extra_usuario($id_extra)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $subconsulta = "(SELECT id_extra FROM gnupanel_usuarios_precios_extras WHERE id_dueno = $id_reseller AND id_extra = $id_extra)";
    $consulta = "SELECT * from gnupanel_usuario_extras WHERE id_extra = $subconsulta ";
    $res_consulta = pg_query($conexion,$consulta);
...
function borrar_extra_usuario($id_extra)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $consulta = "DELETE from gnupanel_usuarios_precios_extras WHERE id_dueno = $id_reseller AND id_extra = $id_extra ";
    $res_consulta = pg_query($conexion,$consulta);
...
function borrar_extra_usuario_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$cantidad = cantidad_extras_usuario();
	if(!isset($comienzo)) $comienzo = 0;
	$extras = lista_extras_usuario($comienzo);
...
function borrar_extra_usuario_1($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$comienzo = $_POST['comienzo'];
	$extra = dame_extra_usuario($id_extra);
...
function borrar_extra_usuario_2($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$id_extra = $_POST['id_extra'];	
	$fue_exito = NULL;
	$verifica = verifica_borrar_extra_usuario($id_extra);

	if($verifica)
	{
		$fue_exito = borrar_extra_usuario($id_extra);
	}
...
function borrar_extra_usuario_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		borrar_extra_usuario_1($nombre_script,NULL);
		break;
		case "2":
		borrar_extra_usuario_2($nombre_script,NULL);
		break;
		default:
		borrar_extra_usuario_0($nombre_script,NULL);
	}
}
