...
function agregar_plan_usuario($nombre_plan,$vigencia,$id_reseller,$cant_subdominios,$cant_dominios_parking,$espacio_en_disco,$transferencia,$cant_bases_postgres,$cant_bases_mysql,$cant_cuentas_correo,$cant_listas_correo,$cant_cuentas_ftp,$precio,$moneda,$es_publicor)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
	...
	if($row == 0)
		...
		if($row > 0)
		    ...
		    {
    		    pg_free_result($res_consulta);
		    $consulta = "INSERT INTO gnupanel_usuarios_planes(plan,vigencia,id_dueno,subdominios,dominios_parking,espacio,transferencia,bases_postgres,bases_mysql,cuentas_correo,listas_correo,cuentas_ftp,precio,moneda,es_publico) values('$nombre_plan',$vigencia,$id_reseller,$cant_subdominios,$cant_dominios_parking,$espacio_en_disco,$transferencia,$cant_bases_postgres,$cant_bases_mysql,$cant_cuentas_correo,$cant_listas_correo,$cant_cuentas_ftp,$precio,$moneda,$es_publico) ";
		    $res_consulta = pg_query($conexion,$consulta);
		    if(!$res_consulta)
			{
			$retorno = NULL;
...
function verifica_agregar_plan_usuario_0($plan,$vigencia,$moneda,$id_reseller)
{
global $escribir;
$retorno = NULL;
if(!verifica_dato($plan,NULL)) return $escribir['carac_inv']." ";
if(!verifica_dato($vigencia,true)) return $escribir['carac_inv']." ";
if(existe_plan_usuario($id_reseller,$plan,$vigencia,$moneda)) return $escribir['ya_existe']." ";
if($plan=="_default") return $escribir['_default']." ";
return $retorno;
}

function verifica_agregar_plan_usuario_1($subdominios,$dominios_parking,$espacio,$transferencia,$bases_postgres,$bases_mysql,$cuentas_correo,$listas_correo,$cuentas_ftp,$precio,$moneda)
{
global $escribir;
$retorno = NULL;
...
function agregar_plan_usuario_0($procesador,$mensaje)
{
    global $idioma;
    global $escribir;
    global $plugin;
    ...
    genera_fila_formulario('moneda',$monedas,'select_pais',$moneda,!$mensaje);
...
function agregar_plan_usuario_1($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$moneda = trim($_POST['moneda']);
	$mostrar_moneda = dame_moneda($moneda);
	$es_publico = trim($_POST['es_publico']);
	$checkeo = verifica_agregar_plan_usuario_0($plan,$vigencia,$moneda,$id_reseller);
	if($checkeo)
		{
		agregar_plan_usuario_0($procesador,$checkeo);
		}
	else
		{
		print "<div id=\"formulario\" > \n";
		...
		genera_fila_formulario('moneda',$mostrar_moneda,'text_blocked_int',20,!$mensaje);
		...
		genera_fila_formulario('id_moneda',$moneda,'hidden',20,!$mensaje);
...
function agregar_plan_usuario_2($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$checkeo = verifica_agregar_plan_usuario_1($subdominios,$dominios_parking,$espacio,$transferencia,$bases_postgres,$bases_mysql,$cuentas_correo,$listas_correo,$cuentas_ftp,$precio,$moneda);
	if($checkeo)
		{
		agregar_plan_usuario_1($procesador,$checkeo);
		}
	else
		{
		$ingreso = agregar_plan_usuario($plan,$vigencia,$id_reseller,$subdominios,$dominios_parking,$espacio,$transferencia,$bases_postgres,$bases_mysql,$cuentas_correo,$listas_correo,$cuentas_ftp,$precio,$moneda,$es_publico);
		print "<div id=\"formulario\" > \n";
		if($ingreso)
		{
...
function agregar_plan_usuario_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		agregar_plan_usuario_1($nombre_script,NULL);
		break;

		case "2":
		agregar_plan_usuario_2($nombre_script,NULL);
		break;

		default:
		agregar_plan_usuario_0($nombre_script,NULL);
	}
}
