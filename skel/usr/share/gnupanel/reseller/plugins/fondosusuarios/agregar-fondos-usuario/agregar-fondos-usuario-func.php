...
function dame_deuda_usuario($id_usuario)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
...
function verifica_agregar_fondos_usuario($fondos)
{
    global $escribir;
	$retorno = NULL;
	if(!verifica_dato($fondos,true)) $retorno = $escribir['carac_inv']." ";
	return $retorno;
}

function agregar_fondos_usuario($id_usuario,$fondos)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $usuario = pg_fetch_result($res_consulta,0,0)."@".pg_fetch_result($res_consulta,0,1);

    $consulta = "BEGIN";
    $res_consulta = pg_query($conexion,$consulta);

    $consulta = "UPDATE gnupanel_divisas_usuario SET credito = credito + $fondos WHERE id_usuario = $id_usuario ";
    $res_consulta = $res_consulta && pg_query($conexion,$consulta);

    $usuario = date('YmdHis')."_usuario_".$usuario;
    $codigo_seguimiento = crc32($usuario);

    if($codigo_seguimiento<0) $codigo_seguimiento = (-1)*$codigo_seguimiento;
    $consulta = "INSERT INTO pagos_usuario(id_usuario,importe,acreditacion_pago,empresa_cobranza,codigo_seguimiento,confirmado,acreditado) VALUES($id_usuario,$fondos,now(),'manual','$codigo_seguimiento',1,1) ";
...
function agregar_fondos_usuario_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function agregar_fondos_usuario_1($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	if(!isset($_POST['fondos'])) $fondos = dame_deuda_usuario($id_usuario);
	...
	genera_fila_formulario('fondos',$fondos,'text_int',8,NULL,true,NULL,254,dame_divisa_usuario($id_usuario));
	...
	genera_fila_formulario("agregar",NULL,'submit',NULL,true,NULL);
...
function agregar_fondos_usuario_2($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$checkeo = NULL;
	$comienzo = $_POST['comienzo'];
	$checkeo = verifica_agregar_fondos_usuario($fondos);

	if($checkeo)
	{
	agregar_fondos_usuario_1($nombre_script,$checkeo);
	}
	else
	{
	$escriba = NULL;
	if(agregar_fondos_usuario($id_usuario,$fondos))
	{
...
function agregar_fondos_usuario_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		agregar_fondos_usuario_1($nombre_script,NULL);
		break;
		case "2":
		agregar_fondos_usuario_2($nombre_script,NULL);
		break;
		default:
		agregar_fondos_usuario_0($nombre_script,NULL);
	}
}
