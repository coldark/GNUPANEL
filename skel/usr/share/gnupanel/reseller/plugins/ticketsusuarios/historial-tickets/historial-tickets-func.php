...
function cantidad_tickets_respondidos()
    ...
    $consulta = "SELECT * from gnupanel_tickets_usuarios WHERE id_cliente_de = $id_reseller AND atendido = 1 AND conforme = 1 ";
...
function lista_tickets_respondidos($comienzo)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id,id_ticket,id_usuario,asunto FROM gnupanel_tickets_usuarios WHERE id_cliente_de = $id_reseller AND atendido = 1 AND conforme = 1 ORDER BY id LIMIT $cant_max_result OFFSET $comienzo";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function dame_ticket_respondido($id)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id,id_ticket,id_usuario,generado,asunto,texto_p,texto_r FROM gnupanel_tickets_usuarios WHERE id = $id ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	return NULL;
