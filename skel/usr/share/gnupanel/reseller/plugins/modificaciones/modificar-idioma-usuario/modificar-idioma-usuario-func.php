...
function dame_idioma_usuario($id_usuario)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT * FROM gnupanel_usuario_lang WHERE id_usuario = $id_usuario ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	return NULL;
...
function modificar_idioma_usuario($id_usuario,$idioma)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "UPDATE gnupanel_usuario_lang SET idioma = '$idioma' WHERE id_usuario = $id_usuario";
    $res_consulta = pg_query($conexion,$consulta);
...
function modificar_idioma_usuario_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function modificar_idioma_usuario_1($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$comienzo = $_POST['comienzo'];
	$idioma_u = trim($_POST['idioma_u']);
	if(strlen($idioma_u)<2) $idioma_u = dame_idioma_usuario($id_usuario);
	...
	genera_fila_formulario('idioma_u',$idiomas,$tipo_form,$idioma_u,NULL);
...
function modificar_idioma_usuario_2($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$escriba = NULL;
	if(modificar_idioma_usuario($id_usuario,$idioma_u))
	{
...
function modificar_idioma_usuario_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		modificar_idioma_usuario_1($nombre_script,NULL);
		break;
		case "2":
		modificar_idioma_usuario_2($nombre_script,NULL);
		break;
		default:
		modificar_idioma_usuario_0($nombre_script,NULL);
	}
}
