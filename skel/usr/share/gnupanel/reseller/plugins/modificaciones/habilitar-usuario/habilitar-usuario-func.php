...
function cantidad_usuarios_usuario()
    ...
    $consulta = "SELECT * from gnupanel_usuario WHERE cliente_de = $id_reseller AND active = 0";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function lista_usuarios_usuario($comienzo)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id_usuario,usuario,dominio FROM gnupanel_usuario WHERE cliente_de = $id_reseller AND active = 0 ORDER BY dominio LIMIT $cant_max_result OFFSET $comienzo";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function habilitar_usuario($id_usuario,$web,$ftp,$correo)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $consulta = "UPDATE gnupanel_usuario SET active = 1, password = ltrim(password,'!') WHERE id_usuario = $id_usuario ";
    $res_consulta = pg_query($conexion,$consulta);
    $checkeo = $checkeo && $res_consulta;

    $consulta = "UPDATE gnupanel_apacheconf SET active = 1, estado = 1 WHERE id_dominio = $id_usuario ";
    $res_consulta = pg_query($conexion,$consulta);
    $checkeo = $checkeo && $res_consulta;

    $consulta = "UPDATE gnupanel_postfix_mailuser SET active = 1, passwd = ltrim(passwd,'!') WHERE id_dominio = $id_usuario ";
    $res_consulta = pg_query($conexion,$consulta);
    $checkeo = $checkeo && $res_consulta;

    $consulta = "UPDATE gnupanel_proftpd_ftpuser SET active = 1, passwd = ltrim(passwd,'!') WHERE id_dominio = $id_usuario ";
    $res_consulta = pg_query($conexion,$consulta);
    $checkeo = $checkeo && $res_consulta;
...
function habilitar_usuario_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function habilitar_usuario_1($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function habilitar_usuario_2($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$checkeo = habilitar_usuario($id_usuario,NULL,NULL,NULL);
	print "<div id=\"formulario\" > \n";
	if($checkeo)
	{
	...
	$enviar = $enviar.$escribir['correo_02']." http://gnupanel.".$dominio_reseller."/users \n";
...
function habilitar_usuario_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		habilitar_usuario_1($nombre_script,NULL);
		break;
		case "2":
		habilitar_usuario_2($nombre_script,NULL);
		break;
		default:
		habilitar_usuario_0($nombre_script,NULL);
	}
}
