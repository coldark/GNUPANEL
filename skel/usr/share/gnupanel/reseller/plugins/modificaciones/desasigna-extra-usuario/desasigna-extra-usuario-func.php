...
function dame_extras_con_vigencia($id_usuario)
{
	global $servidor_db;
	global $puerto_db;
	global $database;
	...
	$consulta = "SELECT * FROM gnupanel_usuarios_precios_extras WHERE id_dueno = $id_reseller AND EXISTS (SELECT * FROM gnupanel_usuario_extras WHERE id_usuario = $id_usuario AND gnupanel_usuario_extras.id_extra = gnupanel_usuarios_precios_extras.id_extra )";
	$res_consulta = pg_query($conexion,$consulta);
	...
	if(is_array($extras_in))
	{
		...
		$mostrar = $escribir[$valores['servicio']]." ".$escribir['cantidad']." (".$escribir['periodo']." ".$valores['periodo']." ".$moneda." ".$valores['precio'].")";
...
function cantidad_usuarios_usuario()
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT * from gnupanel_usuario WHERE cliente_de = $id_reseller AND EXISTS (SELECT * FROM gnupanel_usuario_extras WHERE gnupanel_usuario_extras.id_usuario = gnupanel_usuario.id_usuario) ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function lista_usuarios_usuario($comienzo)
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id_usuario,usuario,dominio FROM gnupanel_usuario WHERE cliente_de = $id_reseller AND EXISTS (SELECT * FROM gnupanel_usuario_extras WHERE gnupanel_usuario_extras.id_usuario = gnupanel_usuario.id_usuario) ORDER BY dominio LIMIT $cant_max_result OFFSET $comienzo";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function desasigna_extra_usuario($id_usuario,$id_extra)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    if($servicio == 'espacio')
	{
	$consulta = "UPDATE gnupanel_espacio SET tope = (SELECT espacio FROM gnupanel_usuario_plan WHERE id_usuario = $id_usuario) WHERE id_dominio = $id_usuario ";
	$res_consulta = pg_query($conexion,$consulta);
	$checkeo = $checkeo &&  $res_consulta;

	$consulta = "UPDATE gnupanel_proftpd_ftpquotalimits SET bytes_in_avail = 1048576*(SELECT espacio FROM gnupanel_usuario_plan WHERE id_usuario = $id_usuario) WHERE EXISTS (SELECT userid FROM gnupanel_proftpd_ftpuser WHERE id_dominio = $id_usuario AND gnupanel_proftpd_ftpuser.userid = gnupanel_proftpd_ftpquotalimits.name ) ";
	$res_consulta = pg_query($conexion,$consulta);
	$checkeo = $checkeo &&  $res_consulta;
	}
    ...
    $consulta = "DELETE FROM gnupanel_usuario_extras WHERE id_usuario = $id_usuario AND id_extra = $id_extra ";
    $res_consulta = pg_query($conexion,$consulta);
    $checkeo = $checkeo &&  $res_consulta;
...
function desasigna_extra_usuario_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function desasigna_extra_usuario_1($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$extras = NULL;
	$extras = array();
	$extras = dame_extras_con_vigencia($id_usuario);
...
function desasigna_extra_usuario_2($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	if(desasigna_extra_usuario($id_usuario,$id_extra))
	{
...
function desasigna_extra_usuario_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		desasigna_extra_usuario_1($nombre_script,NULL);
		break;
		case "2":
		desasigna_extra_usuario_2($nombre_script,NULL);
		break;
		default:
		desasigna_extra_usuario_0($nombre_script,NULL);
	}
}
