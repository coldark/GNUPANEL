...
function verifica_modificar_usuario_usuario($id_usuario,$correo_contacto)
{
    global $escribir;
	$retorno = NULL;
...
function modificar_usuario_usuario($id_usuario,$correo_contacto)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "UPDATE gnupanel_usuario SET correo_contacto = '$correo_contacto' WHERE id_usuario = $id_usuario ";
    $res_consulta = pg_query($conexion,$consulta);
...
function modificar_usuario_usuario_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function modificar_usuario_usuario_1($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
...
function modificar_usuario_usuario_2($nombre_script,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$checkeo = NULL;
	$comienzo = $_POST['comienzo'];
	$checkeo = verifica_modificar_usuario_usuario($id_usuario,$correo_contacto);

	if($checkeo)
	{
	modificar_usuario_usuario_1($nombre_script,$checkeo);
	}
	else
	{
	$escriba = NULL;
	if(modificar_usuario_usuario($id_usuario,$correo_contacto))
	{
...
function modificar_usuario_usuario_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		modificar_usuario_usuario_1($nombre_script,NULL);
		break;
		case "2":
		modificar_usuario_usuario_2($nombre_script,NULL);
		break;
		default:
		modificar_usuario_usuario_0($nombre_script,NULL);
	}
}
