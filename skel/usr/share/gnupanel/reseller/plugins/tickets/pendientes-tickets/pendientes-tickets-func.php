...
function cantidad_tickets_pendientes()
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $consulta = "SELECT * from gnupanel_tickets_reseller WHERE id_reseller = $id_reseller AND atendido = 0 AND conforme = 0 ";
...
function lista_tickets_pendientes($comienzo)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id,id_ticket,asunto FROM gnupanel_tickets_reseller WHERE id_reseller = $id_reseller AND atendido = 0 AND conforme = 0 ORDER BY id LIMIT $cant_max_result OFFSET $comienzo";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function dame_ticket_pendiente($id)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT id_ticket,id_reseller,generado,asunto,texto_p FROM gnupanel_tickets_reseller WHERE id = $id ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	return NULL;
    ...
    $retorno['generado'] = substr($retorno['generado'],0,strpos($retorno['generado'],"."));
...
function pendientes_tickets_0($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$cantidad = cantidad_tickets_pendientes();
	if(!isset($comienzo)) $comienzo = 0;
	$tickets = lista_tickets_pendientes($comienzo);
...
function pendientes_tickets_1($procesador,$mensaje)
{
	global $idioma;
	global $escribir;
	global $plugin;
	...
	$comienzo = $_POST['comienzo'];
	if(!isset($comienzo)) $comienzo = 0;
	$ticket_data = dame_ticket_pendiente($id);
	print "<div id=\"formulario\" > \n";
	...
	print "<table width=\"90%\" > \n";
		...
		print "<tr> \n";
		print "<td> \n";
		$escriba = $escribir['generado'];
		print "$escriba \n";
		print "</td> \n";
		print "<td> \n";
		$escriba = $ticket_data['generado'];
		print "$escriba \n";
		print "</td> \n";
		print "</tr> \n";
...
function pendientes_tickets_init($nombre_script)
{
	global $_POST;
	...
	{
		case "1":
		pendientes_tickets_1($nombre_script,NULL);
		break;

		default:
		pendientes_tickets_0($nombre_script,NULL);
	}
}
