...
function boton_con_formulario($accion,$texto,$variables,$imagen_fondo,$destino="_self")
{
	print "<form method=\"post\" action= \"$accion\" target=\"$destino\" > \n";
...
function existe_plan_usuario($id_reseller,$nombre_plan,$vigencia,$moneda)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT * from gnupanel_usuarios_planes WHERE id_dueno=$id_reseller AND plan='$nombre_plan' AND vigencia=$vigencia AND moneda = $moneda ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function cadena_valida_caja($cadena_in,$obligatorio=true,$espacios=NULL)
{
    $retorno = NULL;
    $cadena = $cadena_in;
    $patron = "[^-a-zA-Z0-9_\.@?;,:ñÑ/$]";
    if($espacios) $patron = "[^-a-zA-Z0-9_ \.@?;,:ñÑ/$]";
...
function genera_fila_formulario($nombre_var,$valor,$tipo_form,$tam,$en_blanco,$obligatorio=true,$espacios=NULL,$tam_text_ingr=254,$texto_der="")
{
	...
	case 'text_int_may':
		...
		if((verifica_dato($valor,1,$obligatorio,$espacios) && $valor>0) || $en_blanco)
		{
			print "<tr> \n";
			print "<td> \n";
			print $escribir[$nombre_var];
	...
	case 'check_box':
			...
			if($valor=="true")
			{
			print "<input type=\"checkbox\" name=\"$nombre_var\" value=\"true\" checked > \n";
			}
			else
			{
			print "<input type=\"checkbox\" name=\"$nombre_var\" value=\"true\" > \n";
			}
...
function dame_tema_interfaz_reseller()
{
    global $_SERVER;
    global $_SESSION;
    ...
    $result = "estilos/".$tema_default_config."/estilos";
    if($dominio)
	{
	if(file_exists("estilos/personalizados/".$dominio)) $result = "estilos/personalizados/".$dominio."/estilos";
	}
    ...
    if($logueado==1) $result = NULL;

    return $result;
}

function dame_tema_interfaz_reseller_per()
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    if($logueado==1)
	{
		$consulta = "SELECT id_reseller from gnupanel_reseller WHERE dominio = '$dominio' " ;
		$res_consulta = pg_query($conexion,$consulta);
		$checkeo = $res_consulta;
		if(pg_num_rows ($res_consulta)>0) $id_reseller = pg_fetch_result($res_consulta,0,0);

		if($id_reseller)
		{
			$consulta = "SELECT id_tema from gnupanel_reseller_sets WHERE id_reseller = $id_reseller " ;
			$res_consulta = pg_query($conexion,$consulta);
			$checkeo = $checkeo && $res_consulta;
			...
			if($tema)
			{
				$result = "estilos/".$tema."/estilos";
			}
		}
    		else
		{
			$result = "estilos/".$tema_default_config."/estilos";
			if(dame_tema()) $result = "estilos/".dame_tema()."/estilos";
		}
...
function dame_tema()
{
    ...
    $consulta = "SELECT tema from gnupanel_temas WHERE id_tema = (SELECT id_tema from gnupanel_reseller_sets WHERE id_reseller = $id_reseller) " ;
    $res_consulta = pg_query($conexion,$consulta);
    if(pg_num_rows($res_consulta)>0) $tema = pg_fetch_result($res_consulta,0,0);
    $result = $tema;
    pg_close($conexion);
    return $result;
}
...
function dame_planes_usuario()
{
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT DISTINCT plan from gnupanel_usuarios_planes WHERE id_dueno=$id_reseller ORDER BY plan";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function dame_vigencias_plan($plan)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT vigencia from gnupanel_usuarios_planes WHERE id_dueno=$id_reseller AND plan = '$plan' ORDER BY vigencia ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function dame_monedas_plan_vigencia($plan,$vigencia)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT moneda from gnupanel_usuarios_planes WHERE id_dueno=$id_reseller AND plan = '$plan' AND vigencia = $vigencia ORDER BY moneda ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
foreach($retorno as $devolver)
	{
	$result[$devolver['moneda']] = dame_moneda($devolver['moneda']);
	}
}
...
function corresponde_vigencia($plan,$vigencia)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT count(id_plan) from gnupanel_usuarios_planes WHERE id_dueno = $id_reseller AND plan = '$plan' AND vigencia = $vigencia ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
if($retorno>0) $result = true;
pg_close($conexion);
return $result;
}

function corresponde_monedas_plan_vigencia($plan,$vigencia,$moneda)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT count(id_plan) from gnupanel_usuarios_planes WHERE id_dueno = $id_reseller AND plan = '$plan' AND vigencia = $vigencia AND moneda = $moneda ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
...
function dame_precio_plan($plan,$vigencia,$moneda)
{
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT precio,moneda from gnupanel_usuarios_planes WHERE id_dueno=$id_reseller AND plan = '$plan' AND vigencia = $vigencia AND moneda = $moneda ";
    $res_consulta = pg_query($conexion,$consulta);
    if(!$res_consulta)
	{
	$retorno = NULL;
	...
	{
	$precio = pg_fetch_result($res_consulta,0,0);
	$id_moneda = pg_fetch_result($res_consulta,0,1);
	}
    $consulta = "SELECT moneda from gnupanel_monedas WHERE id_moneda = $id_moneda ";
    $res_consulta = pg_query($conexion,$consulta);
    $moneda = pg_fetch_result($res_consulta,0,0);
    $result['precio'] = $precio;
    $result['id_moneda'] = $id_moneda;
...
function dame_dias_de_gracia_reseller($id_reseller)
    {
    global $servidor_db;
    global $puerto_db;
    global $database;
    ...
    $conectar = "host=$servidor_db dbname=$database user=$usuario_db password=$passwd_db port=$puerto_db";
    $conexion = pg_connect($conectar,PGSQL_CONNECT_FORCE_NEW) OR die("No es posible conectarse con la base de datos");
    $consulta = "SELECT dias_de_gracia FROM gnupanel_reseller_sets WHERE id_reseller = $id_reseller ";
    $res_consulta = pg_query($conexion,$consulta);
    $retorno = pg_fetch_result ($res_consulta,0,0);
    pg_free_result($res_consulta);
    pg_close($conexion);
